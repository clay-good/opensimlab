...
function head(route: (typeof ROUTES)[number], styles: string): string {
  ...
  return [
    ...
    // Declared so a scraper can lay the card out before it has fetched the image,
    // and described so the preview is not silent to a screen reader.
...
function main(): void {
  ...
  const robots = INDEXABLE ? robotsTxt() : [
    '# Generated by `npm run prerender`. Do not edit.',
    '#',
...
/**
 * The robots file for a build that invites indexing.
 *
 * Exported so a test can read the rules it actually writes. A robots path is a
 * PREFIX match, which is the whole reason the generated rules are anchored with
 * `$`: a bare `Disallow: /review` also blocked `/review-status`, an indexable
 * page listed in the very sitemap this file names.
 */
export function robotsTxt(): string {
  return [
    ...
    // These two stay unanchored on purpose: they are not routes but transient
    // cockpit states, and a prefix keeps covering them if they ever take an id.
