...
**opensimlab.com/anesthesia** is the first complete 39-scenario module. Emergency medicine is complete at `/emergency-medicine` with 25 bounded labs. Critical care is complete at `/critical-care` with 24 bounded labs. Cardiology is now playable at `/cardiology` with bounded stable chest-pain, clinic STEMI first-actions, NSTEMI risk-reassessment, acute decompensated heart-failure, and atrial-fibrillation labs.
