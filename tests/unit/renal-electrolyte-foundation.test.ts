...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(217);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(217);
    expect(ROUTES).toHaveLength(243);
    expect(indexableRoutes()).toHaveLength(240);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(225);
