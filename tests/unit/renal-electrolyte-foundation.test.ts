...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    expect(availableModules()).toHaveLength(15);
    expect(READY_MODULE_COUNT).toBe(15);
    expect(READY_SCENARIO_COUNT).toBe(231);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(231);
    expect(ROUTES).toHaveLength(259);
    expect(indexableRoutes()).toHaveLength(256);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(241);
