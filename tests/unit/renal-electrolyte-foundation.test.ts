...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(224);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(224);
    expect(ROUTES).toHaveLength(251);
    expect(indexableRoutes()).toHaveLength(248);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(233);
