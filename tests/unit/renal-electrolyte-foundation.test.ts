...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(232);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(232);
    expect(ROUTES).toHaveLength(260);
    expect(indexableRoutes()).toHaveLength(257);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(242);
