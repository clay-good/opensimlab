...
describe('Renal and Electrolyte Medicine module foundation', () => {
  it('registers two separate previews toward twelve planned lessons without changing the default', () => {
    ...
    expect(RENAL_ELECTROLYTE_SCENARIOS).toHaveLength(2);
    expect(RENAL_ELECTROLYTE_SCENARIOS.map(({ metadata }) => metadata.id)).toEqual([
      id, 'hypokalemia-magnesium-and-ongoing-losses',
    ]);
  ...
  it('mounts the live module and preserves unknown-address feedback', async () => {
    ...
    expect(directory).toContain('2 of 12 planned Renal and Electrolyte Medicine labs');
    expect(directory).toContain(`href="${path}"`);
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(206);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(206);
    expect(ROUTES).toHaveLength(231);
    expect(indexableRoutes()).toHaveLength(228);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(completion).toMatchObject({ moduleId: 'renal-electrolyte', scenarioCount: 2, completeScenarioCount: 0 });
    expect(completion.scenarios).toHaveLength(2);
    expect(quality).toMatchObject({ moduleId: 'renal-electrolyte', scenarioCount: 2, playableScenarioCount: 0 });
    expect(maturity).toMatchObject({ moduleId: 'renal-electrolyte', recordCount: 2 });
    ...
    expect(reports.scenarios).toHaveLength(214);
    const existing = reports.scenarios.filter((entry: { moduleId: string; scenarioId: string }) =>
