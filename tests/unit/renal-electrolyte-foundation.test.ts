...
describe('Renal and electrolyte medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(248);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(248);
    ...
    expect(ROUTES).toHaveLength(278);
    expect(indexableRoutes()).toHaveLength(275);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(262);
