...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(233);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(233);
    expect(ROUTES).toHaveLength(261);
    expect(indexableRoutes()).toHaveLength(258);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(243);
