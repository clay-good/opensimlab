...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(223);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(223);
    expect(ROUTES).toHaveLength(250);
    expect(indexableRoutes()).toHaveLength(247);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(232);
