...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(234);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(234);
    expect(ROUTES).toHaveLength(262);
    expect(indexableRoutes()).toHaveLength(259);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(244);
