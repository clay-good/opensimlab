...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(221);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(221);
    expect(ROUTES).toHaveLength(248);
    expect(indexableRoutes()).toHaveLength(245);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(230);
