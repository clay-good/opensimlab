...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(216);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(216);
    expect(ROUTES).toHaveLength(242);
    expect(indexableRoutes()).toHaveLength(239);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(224);
