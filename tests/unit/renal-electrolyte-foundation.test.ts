...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(222);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(222);
    expect(ROUTES).toHaveLength(249);
    expect(indexableRoutes()).toHaveLength(246);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(231);
