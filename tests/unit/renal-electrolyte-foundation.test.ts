...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(213);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(213);
    expect(ROUTES).toHaveLength(239);
    expect(indexableRoutes()).toHaveLength(236);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(221);
