...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(226);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(226);
    expect(ROUTES).toHaveLength(253);
    expect(indexableRoutes()).toHaveLength(250);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(235);
