...
describe('Renal and Electrolyte Medicine module foundation', () => {
  ...
  it('keeps registry, landing, routes, and published artifact counts aligned', () => {
    ...
    expect(READY_SCENARIO_COUNT).toBe(225);
    expect(reviewableItems().filter((item) => item.kind === 'scenario')).toHaveLength(225);
    expect(ROUTES).toHaveLength(252);
    expect(indexableRoutes()).toHaveLength(249);
  ...
  it('publishes exact preview, quality-gap, maturity, and secure-report artifacts', () => {
    ...
    expect(reports.scenarios).toHaveLength(234);
