...
describe('Nursing module foundation', () => {
  it('registers seven previews toward nine planned lessons', () => {
    ...
    expect(MEDICAL_SURGICAL_NURSING_SCENARIOS).toHaveLength(7);
  ...
  it('is reachable from the nav, the routes, and the prerendered markup', () => {
    ...
    expect(ROUTES.filter((entry) => entry.path.startsWith('/medical-surgical-nursing'))).toHaveLength(8);
