...
describe('Nursing module foundation', () => {
  it('registers six previews toward nine planned lessons', () => {
    ...
    expect(MEDICAL_SURGICAL_NURSING_SCENARIOS).toHaveLength(6);
  ...
  it('is reachable from the nav, the routes, and the prerendered markup', () => {
    ...
    expect(ROUTES.filter((entry) => entry.path.startsWith('/medical-surgical-nursing'))).toHaveLength(7);
  ...
  it('publishes structured data and its own catalog artifacts', () => {
    ...
    expect(completion.scenarioCount).toBe(6);
