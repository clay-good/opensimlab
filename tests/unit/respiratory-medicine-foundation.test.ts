...
describe('respiratory medicine module foundation', () => {
  it('registers one available specialty module and one honest scenario registry', () => {
  ...
  it('includes the exact-version scenario in governance and problem-report catalogs', () => {
    ...
    for (const path of [
      ...
      expect(catalog.scenarios.filter((entry) => entry.moduleId === moduleId)).toHaveLength(1);
    }
  });
  ...
  it('publishes completion, quality, and maturity artifacts for the new module', () => {
    ...
    expect(completion).toMatchObject({ moduleId, scenarioCount: 1 });
    expect(completion.scenarios).toEqual([
    ...
    expect(quality).toMatchObject({ moduleId, scenarioCount: 1 });
    ...
    expect(maturity).toMatchObject({ moduleId, recordCount: 1 });
