...
describe('Requirement: Every Audited Example Is Offered', () => {
  ...
  it('offers every anesthesia example and nothing else in the module', () => {
    ...
    // offers four, and they do not all work the same way: rapid-desaturation,
    // hypotension-after-induction and rapid-sequence-induction are
    ...
    // exist. The remaining thirty-five labs offer nothing.
    ...
    expect(offered.slice().sort()).toEqual([
      'hypotension-after-induction', 'rapid-desaturation', 'rapid-sequence-induction',
      'routine-induction',
    ]);
  });
