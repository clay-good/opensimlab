...
describe('Requirement: Every Audited Example Is Offered', () => {
  ...
  it('offers every anesthesia example and nothing else in the module', () => {
    ...
    // offers three, and they do not all work the same way: rapid-desaturation
    // and hypotension-after-induction are observed-state like every other
    // example in the catalog, and routine-induction is the older scripted
    ...
    // exist. The remaining thirty-six labs offer nothing.
    ...
    expect(offered.slice().sort()).toEqual([
      'hypotension-after-induction', 'rapid-desaturation', 'routine-induction',
    ]);
  });
