...
describe('Surgery and trauma module foundation', () => {
  it('opens the sixteenth module and leaves nothing declared but unbuilt', () => {
    ...
    expect(SURGERY_TRAUMA_SCENARIOS).toHaveLength(10);
  ...
  it('is reachable from the nav, the routes, and the prerendered markup', () => {
    ...
    expect(ROUTES.filter((entry) => entry.path.startsWith('/surgery-trauma'))).toHaveLength(11);
  ...
  it('publishes structured data and its own catalog artifacts', () => {
    ...
    expect(completion.scenarioCount).toBe(10);
  ...
  it('alternates a lesson about holding with a lesson about not waiting', () => {
    ...
    const [first, second, third, fourth, fifth, sixth, seventh, eighth, ninth, tenth] = SURGERY_TRAUMA_SCENARIOS;
    ...
    expect(tenth!.metadata.id).toBe('unspoken-doubt-an-interval-nobody-else-can-see');
    // Ten of a planned ten: the module's declared scope is now met, and the alternation ran
    // hold, don't-wait five times each without ever repeating a mechanism or a pressure.
    expect(SURGERY_TRAUMA_SCENARIOS).toHaveLength(10);
