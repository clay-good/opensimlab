...
import { RenalHypokalemia } from '../../src/modules/renal-electrolyte/hypokalemia';
import { renalHypokalemiaReportActions as reportActions } from '../../src/modules/renal-electrolyte/hypokalemia-reporting';

const request = (action: string, tick = 3): LearnerAction => ({ tick, type: 'renal-hypokalemia-response', payload: { action } });
const event = (id: string, tick = 3): EngineEvent => ({ tick, eventId: `renal-hypokalemia-${id}-${tick}`,
  category: 'assessment', severity: 'warning', message: 'Private event prose.' });

describe('RenalHypokalemia reports require uniquely attributable action outcomes', () => {
  it.each([
    ['potassium', 'potassium-care', 'accepted'], ['call-support', 'support', 'accepted'],
    ['review-context', 'context-review', 'accepted'], ['manage-losses', 'losses-care', 'accepted'], ['magnesium', 'magnesium-care', 'accepted'], ['check-ecg', 'ecg-check', 'accepted'],
    ['monitor', 'monitoring', 'accepted'], ['check-potassium', 'potassium-check', 'accepted'],
    ['reassess', 'initial-reassessment', 'accepted'], ['reassess', 'recurrence-reassessment', 'accepted'],
    ['reassess', 'potassium-reassessment', 'accepted'], ['reassess', 'response-reassessment', 'accepted'], ['reassess', 'magnesium-reassessment', 'accepted'], ['reassess', 'partial-reassessment', 'accepted'],
    ['handoff', 'handoff', 'accepted'], ['handoff', 'handoff-refused', 'refused'],
    ['rapid-potassium', 'rapid-potassium-refused', 'refused'], ['stop-monitoring', 'monitoring-stop-refused', 'refused'],
  ] as const)('matches %s only to %s', (action, id, outcome) => {
    expect(reportActions([request(action)], [event(id)])).toEqual([
      { tick: 3, type: 'renal-hypokalemia-response', payload: { action }, outcome },
    ]);
  });

  it('independently attributes restoration, observation, and refusals at one tick', () => {
    const model = new RenalHypokalemia();
    const actions = ['potassium', 'rapid-potassium', 'check-potassium', 'stop-monitoring'].map((action) => request(action));
  ...
  it('omits missing, unrelated, wrong-tick, shared, and automatic checkpoint events', () => {
    for (const events of [[], [event('monitoring', 4)], [event('monitoring-duplicate')], [event('action-refused')],
      [event('monitoring-stop-refused')], [event('early-response-checkpoint')], [{ ...event('monitoring'), tick: 4 }]]) {
      expect(reportActions([request('monitor')], events)).toEqual([]);
    }
    const model = new RenalHypokalemia(); model.apply('monitor', 0);
    ...
    expect(refused.at(-1)?.eventId).toBe('renal-hypokalemia-action-refused-3');
  ...
  it('preserves input history and omits event prose and structured private values', () => {
    ...
    expect(reportActions(actions, events)).toEqual([{ tick: 3, type: 'renal-hypokalemia-response', outcome: 'accepted', payload: { action: 'monitor' } }]);
