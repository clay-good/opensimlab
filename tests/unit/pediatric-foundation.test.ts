...
describe('Requirement: bounded healthy-child respiratory physiology', () => {
  ...
  it('keeps the croup 3-year-old, 15 kg scaffold finite and source-composed', () => {
    const profile = healthyChildRespiratoryProfile(3, 15);
    expect(profile.frcLitres).toBeCloseTo(9.51 * 15 ** 1.31 / 1000, 12);
    expect(profile.vo2LitresPerMin).toBeGreaterThan(0);
    expect(profile.vco2LitresPerMin).toBeGreaterThan(0);
    expect(profile.deadSpaceMl).toBeGreaterThan(0);
    expect(profile.spontaneousTidalVolumeMl).toBe(90);
