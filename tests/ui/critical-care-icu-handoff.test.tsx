...
describe('ICU handoff experience', () => {
  ...
  it('fails closed on the timeline target rather than the scenario id', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasIcuHiddenDeteriorationHandoffResponse).toBe(true);
...
describe('ICU handoff tutor and worked example', () => {
  ...
  it('reads the learner’s own recorded steps when guidance is on', () => {
    ...
    const content = markup(READY, { guidance: 'guided' });
  ...
  it('reads the pressure and the EtCO2 the hard way', () => {
    expect(markup(CONTENT, { guidance: 'guided' }))
      .toContain('because the number was held up');
  });

  it('puts names on the tasks', () => {
    expect(markup(CHECKED, { guidance: 'guided' }))
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'IcuHiddenDeteriorationHandoff' });
