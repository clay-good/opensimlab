...
describe('Obstetrics suspected amniotic fluid embolism experience', () => {
  ...
  it('requires exact identity and exposes one calm serial action without treatment or arrest controls', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasObstetricsAfeResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, metadata: { ...SCENARIO.metadata, id: 'suspected-amniotic-fluid-embolism-pattern-lookalike' } }).hasObstetricsAfeResponse).toBe(false);
    const missingBoundary = { ...SCENARIO, timeline: SCENARIO.timeline.filter((event) => event.target !== 'suspected-amniotic-fluid-embolism-pattern-transition-boundary') }; expect(crisisResponseAvailabilityWithTrays(missingBoundary).hasObstetricsAfeResponse).toBe(false);
    const states = [
