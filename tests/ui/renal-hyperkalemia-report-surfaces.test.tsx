...
import { RENAL_HYPERKALEMIA_SHIFT_TICKS, RENAL_HYPERKALEMIA_REBOUND_TICKS } from '../../src/modules/renal-electrolyte/hyperkalemia';
...
describe('Renal hyperkalemia reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4941&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4941, practiceRegion: 'US' });
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'renal-hyperkalemia-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const renalHyperkalemia = frame.equipment.resuscitation.renalHyperkalemia!;
    const privateState = { ...frame.state, glucoseMgDl: 999, potassiumMmolL: 9, cgmGlucoseMgDl: 999, latentBranch: 9 };
    const privateSnapshot = { ...renalHyperkalemia, choiceFeedback: 'private-value', alertness: 'private-value',
      actualGlucoseMgDl: 999, actualPotassiumMmolL: 9, cgmGlucoseMgDl: 999, branch: 'private-branch',
      observation: { ...renalHyperkalemia.observation!, alertness: 'private-value', hiddenGlucoseMgDl: 999 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, renalHyperkalemia: privateSnapshot } },
  ...
  it('collects only explicit observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.renalHyperkalemia.observation.atTick': 23,
      'resuscitation.renalHyperkalemia.observation.glucoseMgDl': 108,
      'resuscitation.renalHyperkalemia.observation.potassiumMmolL': 6.9 });
  ...
  it('keeps glucose-only and full observations separately timestamped without exposing automatic checkpoints', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4941, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'renal-hyperkalemia-response', payload: { action: 'shift' } });
    let frame = engine.step();
    const initial = collectReportEquipmentContext(frame.equipment);
    expect(initial['resuscitation.renalHyperkalemia.observation']).toBeNull();
    expect(initial['resuscitation.renalHyperkalemia.glucoseObservation']).toBeNull();
    expect(Object.keys(initial)).toHaveLength(18);
    expect(Object.keys(initial).some((key) => /MgDl|MmolL/.test(key))).toBe(false);
    engine.apply({ tick: frame.tick, type: 'renal-hyperkalemia-response', payload: { action: 'check-glucose' } });
    frame = engine.step();
    const glucoseOnly = collectReportEquipmentContext(frame.equipment);
    expect(glucoseOnly['resuscitation.renalHyperkalemia.glucoseObservation.atTick']).toBe(1);
    expect(glucoseOnly['resuscitation.renalHyperkalemia.observation']).toBeNull();
    expect(Object.keys(glucoseOnly).some((key) => /potassium/.test(key))).toBe(false);
    engine.apply({ tick: frame.tick, type: 'renal-hyperkalemia-response', payload: { action: 'reassess' } });
    frame = engine.step();
    const historical = collectReportEquipmentContext(frame.equipment);
    expect(historical['resuscitation.renalHyperkalemia.observation.atTick']).toBe(2);
    expect(historical['resuscitation.renalHyperkalemia.glucoseObservation.atTick']).toBe(2);
    for (let tick = frame.tick; tick <= RENAL_HYPERKALEMIA_SHIFT_TICKS; tick += 1) frame = engine.step();
    ...
    expect(earlyGlucose['resuscitation.renalHyperkalemia.glucoseObservation.glucoseMgDl']).toBe(104);
    expect(earlyGlucose['resuscitation.renalHyperkalemia.glucoseObservation.atTick']).toBeGreaterThan(2);
    expect(earlyGlucose['resuscitation.renalHyperkalemia.observation.atTick']).toBe(2);
    expect(earlyGlucose['resuscitation.renalHyperkalemia.observation.potassiumMmolL']).toBe(6.9);
    for (let tick = frame.tick; tick <= RENAL_HYPERKALEMIA_REBOUND_TICKS; tick += 1) frame = engine.step();
    ...
    expect(Object.keys(complete)).toHaveLength(30);
    expect(complete['resuscitation.renalHyperkalemia.observation.glucoseMgDl']).toBe(104);
    expect(complete['resuscitation.renalHyperkalemia.observation.potassiumMmolL']).toBe(6.6);
    expect(complete['resuscitation.renalHyperkalemia.observation.atTick']).toBe(
      complete['resuscitation.renalHyperkalemia.glucoseObservation.atTick']);
    expect(complete['resuscitation.renalHyperkalemia.observation.atTick']).toBeGreaterThan(
      Number(earlyGlucose['resuscitation.renalHyperkalemia.glucoseObservation.atTick']));
    expect(complete['resuscitation.renalHyperkalemia.glucoseObservation']).toBeUndefined();
    expect(complete['resuscitation.renalHyperkalemia.glucoseObservation.glucoseMgDl']).toBe(104);
  });

  it.each([false, true])('attributes observation, potassium shifting, and refused shortcuts independently at one paused tick (reversed: %s)', async (reversed) => {
    const choices = ['check-glucose', 'ecg-resolved', 'shift', 'stop-glucose-monitoring'];
    if (reversed) choices.reverse();
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4941, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'renal-hyperkalemia-response', payload: { action } }));
    ...
    const expected = choices.map((action) => ({
      tick: 0, type: 'renal-hyperkalemia-response', payload: { action },
      outcome: ['ecg-resolved', 'stop-glucose-monitoring'].includes(action) ? 'refused' : 'accepted',
    }));
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, renalHyperkalemia: undefined } },
      rehearsalBranch: { pointId: 'renal-hyperkalemia-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
