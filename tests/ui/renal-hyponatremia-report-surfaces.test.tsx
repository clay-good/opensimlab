...
import { RENAL_HYPONATREMIA_RESCUE_TICKS, RENAL_HYPONATREMIA_ADDITIONAL_RESCUE_TICKS } from '../../src/modules/renal-electrolyte/hyponatremia';
...
describe('Renal hyponatremia reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4961&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4961, practiceRegion: 'US' });
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'renal-hyponatremia-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const renalHyponatremia = frame.equipment.resuscitation.renalHyponatremia!;
    const privateState = { ...frame.state, glucoseMgDl: 999, sodiumMmolL: 999, cgmGlucoseMgDl: 999, latentBranch: 9 };
    const privateSnapshot = { ...renalHyponatremia, choiceFeedback: 'private-value', alertness: 'private-value',
      actualGlucoseMgDl: 999, actualSodiumMmolL: 999, cgmGlucoseMgDl: 999, branch: 'private-branch',
      sodiumObservation: { ...renalHyponatremia.sodiumObservation!, hiddenSodiumMmolL: 999 },
      neurologicObservation: { ...renalHyponatremia.neurologicObservation!, alertness: 'private-value', hiddenCause: 'private-value' },
      observation: { ...renalHyponatremia.observation!, alertness: 'private-value', hiddenGlucoseMgDl: 999 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, renalHyponatremia: privateSnapshot } },
  ...
  it('collects only explicit observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.renalHyponatremia.observation.atTick': 23,
      'resuscitation.renalHyponatremia.observation.changeFromBaselineMmolL': 0,
      'resuscitation.renalHyponatremia.observation.sodiumMmolL': 118 });
    expect(JSON.stringify(context)).not.toMatch(/actualGlucose|actualSodium|hiddenGlucose|hiddenSodium|hiddenCause|cgmGlucose|latentBranch|branch|alertness|choiceFeedback|DueInSeconds|fio2|etco2|paco2|999/);
  ...
  it('keeps sodium-only, neurologic-only, and full observations historical across both authored responses', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4961, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'renal-hyponatremia-response', payload: { action: 'rescue' } });
    let frame = engine.step();
    const project = () => collectReportEquipmentContext(frame.equipment);
    const field = (name: string) => project()[`resuscitation.renalHyponatremia.${name}`];
    const initial = project();
    expect(Object.keys(initial)).toHaveLength(16);
    expect(field('observation')).toBeNull(); expect(field('sodiumObservation')).toBeNull();
    expect(field('neurologicObservation')).toBeNull();
    expect(Object.keys(initial).some((key) => /MmolL|headache|nausea/.test(key))).toBe(false);
    const choose = (action: string) => {
      engine.apply({ tick: frame.tick, type: 'renal-hyponatremia-response', payload: { action } });
      frame = engine.step();
    };
    choose('reassess');
    const historical = project();
    expect(field('observation.atTick')).toBe(1);
    expect(field('observation.sodiumMmolL')).toBe(118);
    for (let tick = frame.tick; tick <= RENAL_HYPONATREMIA_RESCUE_TICKS; tick += 1) frame = engine.step();
    expect(project()).toEqual(historical);
    choose('check-sodium');
    expect(field('sodiumObservation.sodiumMmolL')).toBe(123);
    expect(field('sodiumObservation.changeFromBaselineMmolL')).toBe(5);
    expect(field('observation.sodiumMmolL')).toBe(118);
    expect(field('observation.atTick')).toBe(1);
    expect(field('neurologicObservation.atTick')).toBe(1);
    const sodiumAt = field('sodiumObservation.atTick');
    choose('check-neurology');
    expect(field('sodiumObservation.atTick')).toBe(sodiumAt);
    expect(field('neurologicObservation.atTick')).toBeGreaterThan(Number(sodiumAt));
    expect(field('neurologicObservation.headache')).toBe(true);
    expect(field('neurologicObservation.nausea')).toBe(true);
    expect(field('observation.atTick')).toBe(1);
    expect(field('initialResponseObserved')).toBe(false);
    choose('reassess');
    expect(field('initialResponseObserved')).toBe(true);
    expect(field('persistentSymptomsObserved')).toBe(true);
    expect(field('observation.sodiumMmolL')).toBe(123);
    const fullAt = field('observation.atTick');
    const additionalAt = frame.tick;
    choose('additional-rescue');
    const requested = project();
    for (let tick = frame.tick; tick <= additionalAt + RENAL_HYPONATREMIA_ADDITIONAL_RESCUE_TICKS; tick += 1) frame = engine.step();
    expect(project()).toEqual(requested);
    choose('check-neurology');
    expect(field('observation.atTick')).toBe(fullAt);
    expect(field('sodiumObservation.sodiumMmolL')).toBe(123);
    choose('check-sodium');
    expect(field('sodiumObservation.sodiumMmolL')).toBe(124);
    expect(field('sodiumObservation.changeFromBaselineMmolL')).toBe(6);
    expect(field('observation.sodiumMmolL')).toBe(123);
    expect(field('additionalResponseObserved')).toBe(false);
    choose('reassess');
    expect(Object.keys(project())).toHaveLength(31);
    expect(field('observation.sodiumMmolL')).toBe(124);
    expect(field('observation.headache')).toBe(true); expect(field('observation.nausea')).toBe(true);
    expect(field('observation.atTick')).toBe(field('sodiumObservation.atTick'));
    expect(field('observation.atTick')).toBe(field('neurologicObservation.atTick'));
    expect(field('additionalResponseObserved')).toBe(true);
    expect(JSON.stringify(project())).not.toMatch(/alertness|choiceFeedback|DueInSeconds|phase|urine|osmolality/);
  });

  it.each([false, true])('attributes observation, rescue, and refused conclusions independently at one paused tick (reversed: %s)', async (reversed) => {
    const choices = ['check-sodium', 'normalize-now', 'rescue', 'siadh-now'];
    if (reversed) choices.reverse();
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4961, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'renal-hyponatremia-response', payload: { action } }));
    ...
    const expected = choices.map((action) => ({
      tick: 0, type: 'renal-hyponatremia-response', payload: { action },
      outcome: ['normalize-now', 'siadh-now'].includes(action) ? 'refused' : 'accepted',
    }));
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, renalHyponatremia: undefined } },
      rehearsalBranch: { pointId: 'renal-hyponatremia-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
