...
describe('Toxicology anticholinergic experience', () => {
  ...
  it('fails closed and exposes one calm cognitive action at a time', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasToxicologyAnticholinergicResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasToxicologyAnticholinergicResponse).toBe(false);
    const states = [EMPTY,
...
describe('Anticholinergic tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'Anticholinergic' });
