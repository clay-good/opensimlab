...
describe('Respiratory acute-severe-asthma experience', () => {
  ...
  it('fails closed and never offers a treatment, a device, or a setting', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasAcuteSevereAsthmaResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasAcuteSevereAsthmaResponse).toBe(false);
...
describe('Acute-severe-asthma tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'AcuteSevereAsthma' });
