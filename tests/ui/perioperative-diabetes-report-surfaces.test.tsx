...
import { PERIOPERATIVE_DIABETES_EARLY_TICKS, PERIOPERATIVE_DIABETES_RESPONSE_TICKS } from '../../src/modules/endocrine-metabolic/perioperative-diabetes';
...
describe('Perioperative diabetes reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4927&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4927, practiceRegion: 'US' });
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'perioperative-diabetes-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const perioperativeDiabetes = frame.equipment.resuscitation.perioperativeDiabetes!;
    const privateState = { ...frame.state, glucoseMgDl: 999, ketonesMmolL: 9, cgmGlucoseMgDl: 999, latentBranch: 9 };
    const privateSnapshot = { ...perioperativeDiabetes, choiceFeedback: 'private-value', alertness: 'private-value',
      actualGlucoseMgDl: 999, actualKetonesMmolL: 9, cgmGlucoseMgDl: 999, branch: 'private-branch',
      observation: { ...perioperativeDiabetes.observation!, alertness: 'private-value', hiddenGlucoseMgDl: 999 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, perioperativeDiabetes: privateSnapshot } },
  ...
  it('collects only explicit observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(Object.keys(context.snapshot.patient)).toHaveLength(7); expect(Object.keys(context.snapshot.equipment)).toHaveLength(24);
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.perioperativeDiabetes.observation.atTick': 23,
      'resuscitation.perioperativeDiabetes.observation.glucoseMgDl': 180,
      'resuscitation.perioperativeDiabetes.observation.ketonesMmolL': 0.6 });
    expect(JSON.stringify(context)).not.toMatch(/actualGlucose|actualKetones|hiddenGlucose|cgmGlucose|latentBranch|branch|alertness|choiceFeedback|DueInSeconds|fio2|etco2|paco2/);
  ...
  it('keeps glucose-only and full observations separately timestamped without exposing automatic checkpoints', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4927, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'perioperative-diabetes-response', payload: { action: 'restore-insulin' } });
    let frame = engine.step();
    const initial = collectReportEquipmentContext(frame.equipment);
    expect(initial['resuscitation.perioperativeDiabetes.observation']).toBeNull();
    expect(initial['resuscitation.perioperativeDiabetes.glucoseObservation']).toBeNull();
    expect(Object.keys(initial)).toHaveLength(14);
    expect(Object.keys(initial).some((key) => /MgDl|MmolL/.test(key))).toBe(false);
    engine.apply({ tick: frame.tick, type: 'perioperative-diabetes-response', payload: { action: 'check-glucose' } });
    frame = engine.step();
    const glucoseOnly = collectReportEquipmentContext(frame.equipment);
    expect(glucoseOnly['resuscitation.perioperativeDiabetes.glucoseObservation.atTick']).toBe(1);
    expect(glucoseOnly['resuscitation.perioperativeDiabetes.observation']).toBeNull();
    expect(Object.keys(glucoseOnly).some((key) => /ketones/.test(key))).toBe(false);
    engine.apply({ tick: frame.tick, type: 'perioperative-diabetes-response', payload: { action: 'reassess' } });
    frame = engine.step();
    const historical = collectReportEquipmentContext(frame.equipment);
    expect(historical['resuscitation.perioperativeDiabetes.observation.atTick']).toBe(2);
    expect(historical['resuscitation.perioperativeDiabetes.glucoseObservation.atTick']).toBe(2);
    for (let tick = frame.tick; tick <= PERIOPERATIVE_DIABETES_EARLY_TICKS; tick += 1) frame = engine.step();
    expect(collectReportEquipmentContext(frame.equipment)).toEqual(historical);
    engine.apply({ tick: frame.tick, type: 'perioperative-diabetes-response', payload: { action: 'check-glucose' } });
    frame = engine.step();
    const earlyGlucose = collectReportEquipmentContext(frame.equipment);
    expect(earlyGlucose['resuscitation.perioperativeDiabetes.glucoseObservation.glucoseMgDl']).toBe(162);
    expect(earlyGlucose['resuscitation.perioperativeDiabetes.glucoseObservation.atTick']).toBeGreaterThan(2);
    expect(earlyGlucose['resuscitation.perioperativeDiabetes.observation.atTick']).toBe(2);
    expect(earlyGlucose['resuscitation.perioperativeDiabetes.observation.ketonesMmolL']).toBe(0.6);
    for (let tick = frame.tick; tick <= PERIOPERATIVE_DIABETES_RESPONSE_TICKS; tick += 1) frame = engine.step();
    expect(collectReportEquipmentContext(frame.equipment)).toEqual(earlyGlucose);
    engine.apply({ tick: frame.tick, type: 'perioperative-diabetes-response', payload: { action: 'reassess' } });
    frame = engine.step();
    const complete = collectReportEquipmentContext(frame.equipment);
    expect(Object.keys(complete)).toHaveLength(24);
    expect(complete['resuscitation.perioperativeDiabetes.observation.glucoseMgDl']).toBe(144);
    expect(complete['resuscitation.perioperativeDiabetes.observation.ketonesMmolL']).toBe(0.3);
    expect(complete['resuscitation.perioperativeDiabetes.observation.atTick']).toBe(
      complete['resuscitation.perioperativeDiabetes.glucoseObservation.atTick']);
    expect(complete['resuscitation.perioperativeDiabetes.observation.atTick']).toBeGreaterThan(
      Number(earlyGlucose['resuscitation.perioperativeDiabetes.glucoseObservation.atTick']));
    expect(complete['resuscitation.perioperativeDiabetes.glucoseObservation']).toBeUndefined();
    expect(complete['resuscitation.perioperativeDiabetes.glucoseObservation.glucoseMgDl']).toBe(144);
  });

  it.each([false, true])('attributes observation, insulin restoration, and refused shortcuts independently at one paused tick (reversed: %s)', async (reversed) => {
    const choices = ['check-glucose', 'omit-insulin', 'restore-insulin', 'cgm-only', 'clear-surgery'];
    if (reversed) choices.reverse();
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4927, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'perioperative-diabetes-response', payload: { action } }));
    ...
    const expected = choices.map((action) => ({
      tick: 0, type: 'perioperative-diabetes-response', payload: { action },
      outcome: ['omit-insulin', 'cgm-only', 'clear-surgery'].includes(action) ? 'refused' : 'accepted',
    }));
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, perioperativeDiabetes: undefined } },
      rehearsalBranch: { pointId: 'perioperative-diabetes-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
