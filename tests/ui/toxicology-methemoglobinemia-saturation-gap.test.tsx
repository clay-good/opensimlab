...
describe('Toxicology methemoglobinemia experience', () => {
  ...
  it('fails closed and exposes one calm cognitive action at a time', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasToxicologyMethemoglobinemiaResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasToxicologyMethemoglobinemiaResponse).toBe(false);
    const states = [EMPTY,
...
describe('Methemoglobinemia tutor and worked example', () => {
  ...
  it('keeps the gap as the finding rather than picking a winner', () => {
    const html = markup(EMPTY, { guidance: 'guided' });
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'Methemoglobinemia' });
