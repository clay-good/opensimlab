...
describe('Emergency unstable bradycardia experience', () => {
  ...
  it('fails closed on the timeline target rather than the scenario id', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasUnstableBradycardiaResponse).toBe(true);
...
describe('Emergency unstable bradycardia tutor and worked example', () => {
  ...
  it('reads the learner’s own recorded steps when guidance is on', () => {
    ...
    const support = markup(REVIEWED, { guidance: 'guided' });
  ...
  it('says why positive-pressure ventilation is deliberately not selected', () => {
    expect(markup(REVIEWED, { guidance: 'guided' }))
  ...
  it('refuses to treat a good panel as permission to stop', () => {
    expect(markup(ATROPINE, { guidance: 'guided' }))
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'UnstableBradycardia' });
