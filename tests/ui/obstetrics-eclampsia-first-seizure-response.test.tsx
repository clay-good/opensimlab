...
describe('Obstetrics eclampsia UI', () => {
  ...
  it('requires exact identity and debriefs exact event prefixes', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO)).toMatchObject({ hasObstetricsEclampsiaResponse: true });
...
describe('Obstetrics eclampsia experience', () => {
  ...
  it('fails closed and never offers a maneuver, a scan, or a drug', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasObstetricsEclampsiaResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasObstetricsEclampsiaResponse).toBe(false);
...
describe('Eclampsia tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'ObstetricsEclampsia' });
