...
describe('Obstetrics postpartum-preeclampsia UI', () => {
  ...
  it('requires exact identity and debriefs exact event prefixes', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO)).toMatchObject({ hasObstetricsPostpartumPreeclampsiaResponse: true });
...
describe('Obstetrics postpartum-preeclampsia experience', () => {
  ...
  it('fails closed and never offers a measurement, a question, or a drug', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasObstetricsPostpartumPreeclampsiaResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasObstetricsPostpartumPreeclampsiaResponse).toBe(false);
...
describe('Postpartum-preeclampsia tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'ObstetricsPostpartumPreeclampsia' });
