 ...
 * Only the worker-backed session and canvas/audio cockpit are replaced. Every
 * request is intercepted locally; no Turnstile service, Worker, or D1 is called.
 */
...
describe('exact-version thyroid reporting through shared route surfaces', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4903&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4903, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'thyroid-storm-response', payload: { action: 'reassess' } });
    const frame = engine.step();
    const thyroid = frame.equipment.resuscitation.thyroidStorm!;
    harness.actions = Array.from({ length: 24 }, (_, index) => ({ tick: index + 1,
      type: 'thyroid-storm-response', payload: { action: 'reassess', notes: 'private action prose' } }));
    ...
    harness.session = {
      ...
      log: [{ tick: 24, eventId: 'thyroid-storm-action-refused-24', severity: 'warning', category: 'assessment', message: 'private log prose' }],
      unreadLog: false, guidance: 'guided',
      equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, thyroidStorm: {
        ...thyroid, choiceFeedback: 'private-value', alertness: 'private-value',
  ...
  it('keeps a worked example paused after reporting and includes only explicitly chosen bounded context', async () => {
    ...
    expect(preview).toContain('resuscitation.thyroidStorm'); expect(preview).not.toMatch(/private-|private action|private log|alertness|choiceFeedback/);
    await selectCategory(); await click(button('Send report'));
    const payload = lastPayload(); expectIdentity(payload, 'live');
    expect(payload.recent_context?.seed).toBe(4903);
  ...
  it('keeps a paused replay paused and reports source or replay context without inventing a separate surface', async () => {
    ...
    await render();
    expect(harness.session.rehearseFromDecisionPoint).toHaveBeenCalledWith('thyroid-first-treatment', 1);
    expect(harness.session.transport).toBe('paused');
