...
describe('exact-version hypocalcemia reporting through shared route surfaces', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4906&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4906, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'hypocalcemia-response', payload: { action: 'reassess' } });
    const frame = engine.step();
    const hypocalcemia = frame.equipment.resuscitation.hypocalcemia!;
    harness.actions = Array.from({ length: 24 }, (_, index) => ({ tick: index + 1,
      type: 'hypocalcemia-response', payload: { action: 'reassess', notes: 'private action prose' } }));
    ...
    harness.session = {
      ...
      log: [{ tick: 24, eventId: 'hypocalcemia-action-refused-24', severity: 'warning', category: 'assessment', message: 'private log prose' }],
      unreadLog: false, guidance: 'guided',
      equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, hypocalcemia: {
        ...hypocalcemia, choiceFeedback: 'private-value', symptoms: 'private-value',
        observation: { ...hypocalcemia.observation!, symptoms: 'private-value' },
      } } },
  ...
  it('keeps a worked example paused after reporting and includes only explicitly chosen bounded context', async () => {
    ...
    expect(preview).toContain('resuscitation.hypocalcemia'); expect(preview).not.toMatch(/private-|private action|private log|symptoms|choiceFeedback/);
    await selectCategory(); await click(button('Send report'));
    const payload = lastPayload(); expectIdentity(payload, 'live');
    expect(payload.recent_context?.seed).toBe(4906);
    ...
    expect(payload.recent_context!.snapshot.equipment).toMatchObject({
      'resuscitation.hypocalcemia.observation.atTick': 0,
      'resuscitation.hypocalcemia.observation.adjustedCalciumMgDl': 6.6,
      'resuscitation.hypocalcemia.observation.spo2Percent': 98,
    });
  ...
  it('keeps malformed report action attempts refused without serializing private scalar-like tokens', async () => {
    ...
    harness.actions = [
      { tick: 1, type: 'hypocalcemia-response', payload: { action: 'calcium-rescue', notes: 'private-value' } },
      { tick: 2, type: 'hypocalcemia-response', payload: { action: 'private_value' } },
      { tick: 3, type: 'hypocalcemia-response', payload: { action: 'magnesium', patient_id: 'private_value' } },
      { tick: 4, type: 'hypocalcemia-response', payload: { action: 'reassess' } },
    ];
    ...
    expect(payload.recent_context?.actions).toEqual([
      { tick: 1, type: 'hypocalcemia-response', outcome: 'refused', payload: {} },
      { tick: 2, type: 'hypocalcemia-response', outcome: 'refused', payload: {} },
      { tick: 3, type: 'hypocalcemia-response', outcome: 'refused', payload: {} },
      { tick: 4, type: 'hypocalcemia-response', outcome: 'accepted', payload: { action: 'reassess' } },
    ]);
  ...
  it('keeps a paused replay paused and reports source or replay context without inventing a separate surface', async () => {
    ...
    await render();
    expect(harness.session.rehearseFromDecisionPoint).toHaveBeenCalledWith('hypocalcemia-first-response', 1);
    expect(harness.session.transport).toBe('paused');
