...
describe('Renal hypernatremia reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4973&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4973, practiceRegion: 'US' });
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'renal-hypernatremia-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const renalHypernatremia = frame.equipment.resuscitation.renalHypernatremia!;
    const privateState = { ...frame.state, glucoseMgDl: 999, sodiumMmolL: 999, cgmGlucoseMgDl: 999, latentBranch: 9 };
    const privateSnapshot = { ...renalHypernatremia, choiceFeedback: 'private-value', alertness: 'private-value',
      actualGlucoseMgDl: 999, actualSodiumMmolL: 999, cgmGlucoseMgDl: 999, branch: 'private-branch',
      sodiumObservation: { ...renalHypernatremia.sodiumObservation!, hiddenSodiumMmolL: 999 },
      fluidBalanceObservation: { ...renalHypernatremia.fluidBalanceObservation!, alertness: 'private-value', hiddenUrineOsmolality: 'private-value' },
      observation: { ...renalHypernatremia.observation!, alertness: 'private-value', hiddenGlucoseMgDl: 999 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, renalHypernatremia: privateSnapshot } },
  ...
  it('returns Cancel focus to the report trigger without advancing paused practice or collecting context', async () => {
    ...
    await render();
    const trigger = container.querySelector<HTMLButtonElement>('button[aria-label="Report a problem"]')!;
  ...
  it('collects only explicit observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(Object.keys(context.snapshot.patient)).toHaveLength(7); expect(Object.keys(context.snapshot.equipment)).toHaveLength(32);
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.renalHypernatremia.observation.atTick': 23,
      'resuscitation.renalHypernatremia.observation.changeFromBaselineMmolL': 0,
      'resuscitation.renalHypernatremia.observation.sodiumMmolL': 164,
      'resuscitation.renalHypernatremia.observation.urineOutputMlPerHour': 20,
      'resuscitation.renalHypernatremia.observation.ongoingDiarrhea': true,
      'resuscitation.renalHypernatremia.observation.coreTemperatureC': 37.1 });
    expect(validateReportPayload({ ...payload, recent_context: { ...context, snapshot: { ...context.snapshot,
      equipment: { ...context.snapshot.equipment, extra: 1 } } } }).ok).toBe(false);
    expect(JSON.stringify(context)).not.toMatch(/actualGlucose|actualSodium|hiddenGlucose|hiddenSodium|hiddenUrineOsmolality|cgmGlucose|latentBranch|branch|alertness|choiceFeedback|DueInSeconds|fio2|etco2|paco2|999/);
  ...
  it('keeps sodium-only, fluid-only, and full observations historical through circulation, water response, recurrence, and later care', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4973, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'renal-hypernatremia-response', payload: { action: 'restore-volume' } });
    let frame = engine.step();
    const project = () => collectReportEquipmentContext(frame.equipment);
    const field = (name: string) => project()[`resuscitation.renalHypernatremia.${name}`];
    const initial = project();
    expect(Object.keys(initial)).toHaveLength(17);
    expect(field('observation')).toBeNull(); expect(field('sodiumObservation')).toBeNull();
    expect(field('fluidBalanceObservation')).toBeNull();
    expect(Object.keys(initial).some((key) => /MmolL|urineOutput|ongoingDiarrhea/.test(key))).toBe(false);
    const choose = (action: string) => {
      engine.apply({ tick: frame.tick, type: 'renal-hypernatremia-response', payload: { action } });
      frame = engine.step();
    };
    ...
    expect(field('observation.sodiumMmolL')).toBe(164);
    expect(field('observation.urineOutputMlPerHour')).toBe(20);
    advanceTo(RENAL_HYPERNATREMIA_VOLUME_TICKS);
    expect(project()).toEqual(historical);
    choose('check-fluid-balance');
    expect(field('fluidBalanceObservation.urineOutputMlPerHour')).toBe(35);
    expect(field('fluidBalanceObservation.ongoingDiarrhea')).toBe(true);
    expect(field('observation.urineOutputMlPerHour')).toBe(20);
    expect(field('observation.atTick')).toBe(1);
    expect(field('sodiumObservation.atTick')).toBe(1);
    expect(field('volumeObserved')).toBe(false);
    const waterAt = frame.tick;
    choose('replace-water');
    const requestedWater = project();
    advanceTo(waterAt + RENAL_HYPERNATREMIA_WATER_TICKS);
    expect(project()).toEqual(requestedWater);
    const fluidAt = field('fluidBalanceObservation.atTick');
    choose('check-sodium');
    expect(field('sodiumObservation.sodiumMmolL')).toBe(163);
    expect(field('sodiumObservation.changeFromBaselineMmolL')).toBe(-1);
    expect(field('fluidBalanceObservation.atTick')).toBe(fluidAt);
    expect(field('observation.sodiumMmolL')).toBe(164);
    expect(field('waterResponseObserved')).toBe(false);
    choose('reassess');
    expect(field('waterResponseObserved')).toBe(true);
    expect(field('observation.sodiumMmolL')).toBe(163);
    const waterObserved = project();
    const fullAt = field('observation.atTick');
    advanceTo(waterAt + RENAL_HYPERNATREMIA_RECURRENCE_TICKS);
    expect(project()).toEqual(waterObserved);
    choose('check-fluid-balance');
    expect(field('observation.atTick')).toBe(fullAt);
    expect(field('sodiumObservation.sodiumMmolL')).toBe(163);
    choose('check-sodium');
    expect(field('sodiumObservation.sodiumMmolL')).toBe(164);
    expect(field('sodiumObservation.changeFromBaselineMmolL')).toBe(0);
    expect(field('observation.sodiumMmolL')).toBe(163);
    expect(field('recurrenceObserved')).toBe(false);
    choose('reassess');
    expect(field('recurrenceObserved')).toBe(true);
    const lossesAt = frame.tick;
    choose('manage-losses');
    const requestedLosses = project();
    advanceTo(lossesAt + RENAL_HYPERNATREMIA_COMBINED_TICKS);
    expect(project()).toEqual(requestedLosses);
    choose('check-sodium');
    expect(field('sodiumObservation.sodiumMmolL')).toBe(162);
    expect(field('sodiumObservation.changeFromBaselineMmolL')).toBe(-2);
    expect(field('observation.sodiumMmolL')).toBe(164);
    expect(field('combinedResponseObserved')).toBe(false);
    choose('reassess');
    expect(Object.keys(project())).toHaveLength(32);
    expect(field('observation.sodiumMmolL')).toBe(162);
    expect(field('observation.urineOutputMlPerHour')).toBe(35);
    expect(field('observation.ongoingDiarrhea')).toBe(true);
    expect(field('observation.atTick')).toBe(field('sodiumObservation.atTick'));
    expect(field('observation.atTick')).toBe(field('fluidBalanceObservation.atTick'));
    expect(field('combinedResponseObserved')).toBe(true);
    expect(field('recurrenceObserved')).toBe(true);
  ...
  it.each([false, true])('attributes observation, volume care, and refused shortcuts independently at one paused tick (reversed: %s)', async (reversed) => {
    ...
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4973, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'renal-hypernatremia-response', payload: { action } }));
    ...
    const expected = choices.map((action) => ({
      tick: 0, type: 'renal-hypernatremia-response', payload: { action },
      outcome: ['normalize-now', 'empiric-desmopressin'].includes(action) ? 'refused' : 'accepted',
    }));
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, renalHypernatremia: undefined } },
      rehearsalBranch: { pointId: 'renal-hypernatremia-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
