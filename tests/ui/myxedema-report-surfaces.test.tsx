...
describe('exact-version myxedema reporting through shared route surfaces', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4904&assignment=private-assignment-label`);
    ...
    const myxedema = frame.equipment.resuscitation.myxedema!;
    harness.actions = Array.from({ length: 24 }, (_, index) => ({ tick: index + 1,
      type: 'myxedema-response', payload: { action: 'reassess', notes: 'private action prose' } }));
    ...
    harness.session = {
      ...
      log: [{ tick: 24, eventId: 'myxedema-action-refused-24', severity: 'warning', category: 'assessment', message: 'private log prose' }],
      unreadLog: false, guidance: 'guided',
      equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, myxedema: {
        ...myxedema, choiceFeedback: 'private-value', alertness: 'private-value',
        observation: { ...myxedema.observation!, alertness: 'private-value' },
      } } },
  ...
  it('keeps a worked example paused after reporting and includes only explicitly chosen bounded context', async () => {
    ...
    expect(preview).toContain('resuscitation.myxedema'); expect(preview).not.toMatch(/private-|private action|private log|alertness|choiceFeedback/);
    await selectCategory(); await click(button('Send report'));
    const payload = lastPayload(); expectIdentity(payload, 'live');
    expect(payload.recent_context?.seed).toBe(4904);
    ...
    expect(payload.recent_context!.snapshot.equipment).toMatchObject({
      'resuscitation.myxedema.observation.atTick': 0,
      'resuscitation.myxedema.observation.paco2MmHg': 68,
      'resuscitation.myxedema.observation.spo2Percent': 90,
    });
  ...
  it('keeps a paused replay paused and reports source or replay context without inventing a separate surface', async () => {
    ...
    await render();
    expect(harness.session.rehearseFromDecisionPoint).toHaveBeenCalledWith('myxedema-first-response', 1);
    expect(harness.session.transport).toBe('paused');
