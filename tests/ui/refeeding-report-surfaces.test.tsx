...
import { REFEEDING_ELECTROLYTE_TICKS, REFEEDING_RECURRENCE_TICKS } from '../../src/modules/endocrine-metabolic/refeeding';
...
describe('Refeeding reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'refeeding-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const refeeding = frame.equipment.resuscitation.refeeding!;
    const privateState = { ...frame.state, phosphateMmolL: 0.01, potassiumMmolL: 9, magnesiumMmolL: 9, latentBranch: 9 };
    const privateSnapshot = { ...refeeding, choiceFeedback: 'private-value', alertness: 'private-value',
      actualPhosphateMmolL: 0.01, actualPotassiumMmolL: 9, branch: 'private-branch',
      observation: { ...refeeding.observation!, alertness: 'private-value', hiddenPhosphateMmolL: 0.01 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, refeeding: privateSnapshot } },
  ...
  it('collects only 24 observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.refeeding.observation.atTick': 23,
      'resuscitation.refeeding.observation.phosphateMmolL': 0.30, 'resuscitation.refeeding.observation.potassiumMmolL': 2.7,
      'resuscitation.refeeding.observation.magnesiumMmolL': 0.48 });
    expect(JSON.stringify(context)).not.toMatch(/actualPhosphate|actualPotassium|hiddenPhosphate|latentBranch|branch|alertness|choiceFeedback|DueInSeconds|fio2|etco2|paco2/);
  ...
  it('does not supply initial or later laboratory values without an explicit observation', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4919, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'refeeding-response', payload: { action: 'replace-electrolytes' } });
    let frame = engine.step();
    const initial = collectReportEquipmentContext(frame.equipment);
    expect(initial['resuscitation.refeeding.observation']).toBeNull(); expect(Object.keys(initial)).toHaveLength(14);
    expect(Object.keys(initial).some((key) => /MmolL/.test(key))).toBe(false);
    engine.apply({ tick: frame.tick, type: 'refeeding-response', payload: { action: 'reassess' } }); frame = engine.step();
    const historical = collectReportEquipmentContext(frame.equipment);
    for (let tick = frame.tick; tick <= REFEEDING_ELECTROLYTE_TICKS; tick += 1) frame = engine.step();
    expect(collectReportEquipmentContext(frame.equipment)).toEqual(historical);
    engine.apply({ tick: frame.tick, type: 'refeeding-response', payload: { action: 'reassess' } }); frame = engine.step();
    const observed = collectReportEquipmentContext(frame.equipment);
    expect(observed['resuscitation.refeeding.observation.phosphateMmolL']).toBe(0.50);
    for (let tick = frame.tick; tick <= REFEEDING_RECURRENCE_TICKS; tick += 1) frame = engine.step();
    expect(collectReportEquipmentContext(frame.equipment)).toEqual(observed);
  });

  it.each([false, true])('attributes accepted partial care and refused shortcuts independently at one paused tick (reversed: %s)', async (reversed) => {
    const choices = ['phosphate-only', 'advance-feeding', 'replace-electrolytes', 'stop-monitoring'];
    if (reversed) choices.reverse();
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4919, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'refeeding-response', payload: { action } }));
    ...
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
    const expected = choices.filter((choice) => !reversed || choice !== 'phosphate-only').map((action) => ({
      tick: 0, type: 'refeeding-response', payload: { action },
      outcome: ['advance-feeding', 'stop-monitoring'].includes(action) ? 'refused' : 'accepted',
    }));
    // After complete replacement, phosphate-only receives a shared refusal;
    // ambiguous shared evidence is omitted, never falsely attributed.
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, refeeding: undefined } },
      rehearsalBranch: { pointId: 'refeeding-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
