...
describe('Emergency status epilepticus experience', () => {
  ...
  it('fails closed on the seizure event rather than the scenario id', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasStatusEpilepticusResponse).toBe(true);
...
describe('Emergency status epilepticus tutor and worked example', () => {
  ...
  it('reads the learner’s own recorded steps when guidance is on', () => {
    ...
    const bundle = markup(REVIEWED, { guidance: 'guided' });
  ...
  it('holds the second-line boundary at the reassessment', () => {
    expect(markup(TREATED, { guidance: 'guided' }))
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'StatusEpilepticus' });
