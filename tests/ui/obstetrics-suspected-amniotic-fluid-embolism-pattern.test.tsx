...
describe('Obstetrics amniotic-fluid-embolism UI', () => {
  ...
  it('requires exact identity and debriefs exact event prefixes', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO)).toMatchObject({ hasObstetricsAfeResponse: true });
...
describe('Obstetrics amniotic-fluid-embolism experience', () => {
  ...
  it('fails closed and never offers a maneuver, a product, or a procedure', () => {
    ...
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasObstetricsAfeResponse).toBe(false);
...
describe('Amniotic-fluid-embolism tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'ObstetricsAfe' });
