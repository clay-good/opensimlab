...
describe('Neonatology term-newborn-transition experience', () => {
  ...
  it('fails closed and exposes one calm cognitive action at a time', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasNeonatologyTermTransitionResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasNeonatologyTermTransitionResponse).toBe(false);
...
describe('Term newborn transition tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(start, { guidance: 'guided', demonstratingLessonId: 'TermTransition' });
