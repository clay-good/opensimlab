...
describe('Obstetrics magnesium-toxicity UI', () => {
  ...
  it('requires exact identity and debriefs exact event prefixes', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO, [])).toMatchObject({ hasObstetricsMagnesiumToxicityResponse: true });
...
describe('Obstetrics magnesium-toxicity experience', () => {
  ...
  it('fails closed and never offers an infusion change, an airway, or an antidote', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasObstetricsMagnesiumToxicityResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasObstetricsMagnesiumToxicityResponse).toBe(false);
...
describe('Magnesium-toxicity tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'MagnesiumToxicity' });
