...
describe('Neurology Guillain-Barré experience', () => {
  ...
  it('fails closed and exposes one calm cognitive action at a time', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasNeurologyGbsResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasNeurologyGbsResponse).toBe(false);
    const states = [EMPTY,
