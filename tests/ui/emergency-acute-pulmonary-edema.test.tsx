...
describe('Emergency acute pulmonary edema experience', () => {
  ...
  it('fails closed on the timeline target rather than the scenario id', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasAcutePulmonaryEdemaResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: [] })
...
describe('Emergency acute pulmonary edema tutor and worked example', () => {
  ...
  it('reads the learner’s own recorded steps when guidance is on', () => {
    ...
    const treating = markup(PATTERN, { guidance: 'guided' });
  ...
  it('picks up the missing lane whichever one the learner left', () => {
    expect(markup(DIURETIC_ONLY, { guidance: 'guided' }))
      .toContain('the shortest interval between recording it and the patient looking different');
    expect(markup(SUPPORTED, { guidance: 'guided' }))
      .toContain('comfortably above 110');
    expect(markup(VASODILATED, { guidance: 'guided' }))
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'AcutePulmonaryEdema' });
