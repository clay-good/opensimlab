...
describe('Emergency septic shock experience', () => {
  ...
  it('fails closed on the sepsis-pattern event rather than the scenario id', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasSepticShockResponse).toBe(true);
...
describe('Emergency septic shock tutor and worked example', () => {
  ...
  it('reads the learner’s own recorded steps when guidance is on', () => {
    ...
    const cultures = markup(REVIEWED, { guidance: 'guided' });
  ...
  it('gives the honest reason the fluid course is bounded', () => {
    expect(markup(DOSED, { guidance: 'guided' }))
  ...
  it('names the step the engine never gated', () => {
    expect(markup(PRESSED, { guidance: 'guided' }))
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'SepticShock' });
