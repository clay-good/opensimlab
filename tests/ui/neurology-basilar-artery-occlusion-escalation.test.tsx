...
describe('Neurology basilar-occlusion experience', () => {
  ...
  it('fails closed and exposes one calm cognitive action at a time', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasNeurologyBasilarLvoResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasNeurologyBasilarLvoResponse).toBe(false);
    const states = [EMPTY,
