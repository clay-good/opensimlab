...
describe('Respiratory COPD-transition experience', () => {
  ...
  it('fails closed and never offers a treatment, an eligibility, or a booking', () => {
    expect(crisisResponseAvailabilityWithTrays(SCENARIO).hasCopdTransitionResponse).toBe(true);
    expect(crisisResponseAvailabilityWithTrays({ ...SCENARIO, timeline: SCENARIO.timeline.slice(0, 1) }).hasCopdTransitionResponse).toBe(false);
...
describe('COPD-transition tutor and worked example', () => {
  ...
  it('leaves the controls visible but inert while the example runs', () => {
    ...
    const watching = markup(EMPTY, { guidance: 'guided', demonstratingLessonId: 'CopdTransition' });
