...
describe('Requirement: AKI fluid overload opens a focused demand-capacity surface', () => {
  it('keeps burden, emergency, prescription, and recovery boundaries visible', () => {
    ...
    const props = { scenario: SCENARIO, region: UNITED_STATES, infusions: [],
      hypnoticLine: { connected: true, inspected: false }, resuscitation: {
        ...
        akiFluidOverloadAssessment: { recognitionAtTick: null, contextAtTick: null,
          fluidPlanAtTick: null, supportAtTick: null, reassessmentAtTick: null } },
      lastExposure: null, syringeRemaining: {}, ventilator: { mode: 'volume-control',
        tidalVolumeMl: 420, respiratoryRateBpm: 20, fio2: 0.5, peep: 0, delivering: true,
    ...
    expect(container.textContent).toContain('See the burden. Protect the organs.');
    expect(container.textContent).toContain('Match demand to kidney capacity.');
    expect(container.textContent).toContain('+8.2 L · +9 kg · urine 0.15 mL/kg/h · pulmonary edema');
    expect(container.textContent).toContain('life-threatening imbalance = urgent · no creatinine trigger');
    const recognize = [...container.querySelectorAll('button')]
      .find((button) => button.textContent?.includes('Recognize harmful fluid burden + activate help'));
    act(() => recognize?.click());
    expect(onAction).toHaveBeenCalledWith('recognize-aki-fluid-overload');
