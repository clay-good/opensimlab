...
import { RENAL_HYPOCALCEMIA_RESCUE_TICKS, RENAL_HYPOCALCEMIA_RECURRENCE_TICKS, RENAL_HYPOCALCEMIA_CONTINUING_TICKS } from '../../src/modules/renal-electrolyte/hypocalcemia';
...
describe('Renal hypocalcemia reports through the shared centered form and exact-version Worker validator', () => {
  ...
  beforeEach(() => {
    ...
    history.replaceState({}, '', `${path}?seed=4987&assignment=private-assignment-label`);
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4987, practiceRegion: 'US' });
    harness.actions = Array.from({ length: 24 }, (_, tick) => ({ tick, type: 'renal-hypocalcemia-response', payload: { action: 'reassess' } }));
    const frames = harness.actions.map((action) => { engine.apply(action); return engine.step(); });
    const frame = frames.at(-1)!; const renalHypocalcemia = frame.equipment.resuscitation.renalHypocalcemia!;
    const privateState = { ...frame.state, glucoseMgDl: 999, sodiumMmolL: 999, cgmGlucoseMgDl: 999, latentBranch: 9 };
    const privateSnapshot = { ...renalHypocalcemia, choiceFeedback: 'private-value', alertness: 'private-value',
      actualGlucoseMgDl: 999, actualIonizedCalciumMmolL: 999, totalCalciumMgDl: 999, albuminGdl: 999, adjustedCalciumMgDl: 999, samplePh: 999, phosphateMmolL: 999, magnesiumMmolL: 999, qtcMs: 999, cgmGlucoseMgDl: 999, branch: 'private-branch',
      ionizedObservation: { ...renalHypocalcemia.ionizedObservation!, hiddenIonizedCalciumMmolL: 999 },
      symptomObservation: { ...renalHypocalcemia.symptomObservation!, alertness: 'private-value', hiddenQtcMs: 'private-value' },
      observation: { ...renalHypocalcemia.observation!, alertness: 'private-value', hiddenGlucoseMgDl: 999 } };
    ...
    harness.session = {
      ...
      unreadLog: false, guidance: 'guided', equipment: { ...frame.equipment, resuscitation: { ...frame.equipment.resuscitation, renalHypocalcemia: privateSnapshot } },
  ...
  it('collects only explicit observed equipment fields, seven live vitals, and the last 20 proven actions after opt-in', async () => {
    ...
    expect(Object.keys(context.snapshot.patient)).toHaveLength(7); expect(Object.keys(context.snapshot.equipment)).toHaveLength(30);
    expect(context.snapshot.equipment).toMatchObject({ 'resuscitation.renalHypocalcemia.observation.atTick': 23,
      'resuscitation.renalHypocalcemia.observation.ionizedCalciumMmolL': 0.86,
      'resuscitation.renalHypocalcemia.observation.carpopedalSpasm': true,
      'resuscitation.renalHypocalcemia.observation.perioralTingling': true,
      'resuscitation.renalHypocalcemia.observation.coreTemperatureC': 36.8 });
    ...
    expect(JSON.stringify(context)).not.toMatch(/actualGlucose|actualIonized|hiddenGlucose|hiddenIonized|hiddenQtc|totalCalcium|albuminGdl|adjustedCalcium|samplePh|phosphate|magnesium|qtcMs|cgmGlucose|latentBranch|branch|alertness|choiceFeedback|DueInSeconds|fio2|etco2|paco2|999/);
  ...
  it('keeps ionized-only, symptom-only, and full observations historical through rescue, recurrence, and continuing care', () => {
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4987, practiceRegion: 'US' });
    engine.apply({ tick: 0, type: 'renal-hypocalcemia-response', payload: { action: 'rescue-calcium' } });
    let frame = engine.step();
    const project = () => collectReportEquipmentContext(frame.equipment);
    const field = (name: string) => project()[`resuscitation.renalHypocalcemia.${name}`];
    const initial = project();
    expect(Object.keys(initial)).toHaveLength(17);
    expect(field('observation')).toBeNull(); expect(field('ionizedObservation')).toBeNull();
    expect(field('symptomObservation')).toBeNull();
    expect(Object.keys(initial).some((key) => /MmolL|carpopedalSpasm|perioralTingling/.test(key))).toBe(false);
    const choose = (action: string) => {
      engine.apply({ tick: frame.tick, type: 'renal-hypocalcemia-response', payload: { action } });
      frame = engine.step();
    };
    ...
    expect(field('observation.ionizedCalciumMmolL')).toBe(0.86);
    expect(field('observation.carpopedalSpasm')).toBe(true);
    advanceTo(RENAL_HYPOCALCEMIA_RESCUE_TICKS);
    expect(project()).toEqual(historical);
    choose('check-symptoms');
    expect(field('symptomObservation.carpopedalSpasm')).toBe(false);
    expect(field('symptomObservation.perioralTingling')).toBe(true);
    expect(field('observation.carpopedalSpasm')).toBe(true);
    expect(field('observation.atTick')).toBe(1);
    expect(field('ionizedObservation.atTick')).toBe(1);
    expect(field('rescueResponseObserved')).toBe(false);
    const symptomAt = field('symptomObservation.atTick');
    choose('check-ionized');
    expect(field('ionizedObservation.ionizedCalciumMmolL')).toBe(0.96);
    expect(field('symptomObservation.atTick')).toBe(symptomAt);
    expect(field('observation.ionizedCalciumMmolL')).toBe(0.86);
    expect(field('rescueResponseObserved')).toBe(false);
    choose('reassess');
    expect(field('rescueResponseObserved')).toBe(true);
    expect(field('observation.ionizedCalciumMmolL')).toBe(0.96);
    const rescued = project();
    const fullAt = field('observation.atTick');
    advanceTo(RENAL_HYPOCALCEMIA_RECURRENCE_TICKS);
    expect(project()).toEqual(rescued);
    choose('check-symptoms');
    expect(field('symptomObservation.carpopedalSpasm')).toBe(true);
    expect(field('symptomObservation.perioralTingling')).toBe(true);
    expect(field('observation.atTick')).toBe(fullAt);
    expect(field('ionizedObservation.ionizedCalciumMmolL')).toBe(0.96);
    choose('check-ionized');
    expect(field('ionizedObservation.ionizedCalciumMmolL')).toBe(0.88);
    expect(field('observation.ionizedCalciumMmolL')).toBe(0.96);
    expect(field('recurrenceObserved')).toBe(false);
    choose('reassess');
    expect(field('recurrenceObserved')).toBe(true);
    const continuingAt = frame.tick;
    choose('continue-calcium');
    const requestedContinuing = project();
    advanceTo(continuingAt + RENAL_HYPOCALCEMIA_CONTINUING_TICKS);
    expect(project()).toEqual(requestedContinuing);
    choose('check-ionized');
    expect(field('ionizedObservation.ionizedCalciumMmolL')).toBe(1.03);
    expect(field('observation.ionizedCalciumMmolL')).toBe(0.88);
    expect(field('symptomObservation.carpopedalSpasm')).toBe(true);
    expect(field('continuingResponseObserved')).toBe(false);
    choose('check-symptoms');
    expect(field('symptomObservation.carpopedalSpasm')).toBe(false);
    expect(field('symptomObservation.perioralTingling')).toBe(true);
    expect(field('observation.carpopedalSpasm')).toBe(true);
    expect(field('continuingResponseObserved')).toBe(false);
    choose('reassess');
    expect(Object.keys(project())).toHaveLength(30);
    expect(field('observation.ionizedCalciumMmolL')).toBe(1.03);
    expect(field('observation.carpopedalSpasm')).toBe(false);
    expect(field('observation.perioralTingling')).toBe(true);
    expect(field('observation.atTick')).toBe(field('ionizedObservation.atTick'));
    expect(field('observation.atTick')).toBe(field('symptomObservation.atTick'));
    expect(field('continuingResponseObserved')).toBe(true);
    expect(field('recurrenceObserved')).toBe(true);
  ...
  it.each([false, true])('attributes observation, calcium rescue, and refused shortcuts independently at one paused tick (reversed: %s)', async (reversed) => {
    ...
    const engine = new AnesthesiaEngine({ scenario: SCENARIO, seed: 4987, practiceRegion: 'US' });
    harness.actions = choices.map((action) => ({ tick: 0, type: 'renal-hypocalcemia-response', payload: { action } }));
    ...
    const expected = choices.map((action) => ({
      tick: 0, type: 'renal-hypocalcemia-response', payload: { action },
      outcome: ['trust-adjusted-total', 'oral-only', 'stop-after-relief'].includes(action) ? 'refused' : 'accepted',
    }));
  ...
  it('omits pending and replay-cleared outcomes and never falls back to generic equipment while a snapshot is missing', async () => {
    harness.session = { ...harness.session, phase: 'running', log: [],
      equipment: { ...harness.session.equipment!, resuscitation: { ...harness.session.equipment!.resuscitation, renalHypocalcemia: undefined } },
      rehearsalBranch: { pointId: 'renal-hypocalcemia-first-response', decisionTick: 1, parentTicks: 24 } };
    await render(); await openReport(); await click(container.querySelector<HTMLInputElement>('input[type="checkbox"]')!);
    await selectCategory(); await click(button('Send report')); const payload = lastPayload(); expectIdentity(payload, 'live');
