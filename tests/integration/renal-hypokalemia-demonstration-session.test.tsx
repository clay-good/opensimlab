...
import { RENAL_HYPOKALEMIA_POTASSIUM_TICKS as EARLY,
...
describe('RenalHypokalemia worked example through the real session pipeline', () => {
  ...
  const patient = () => session().equipment?.resuscitation.renalHypokalemia;
  const worker = () => workers.at(-1)!;
  const recorded = () => sessionInternals().recorder!.build('pending').actions;
  function Harness({ active }: { active: boolean }) {
    const state = useSession();
    const demo = useRenalHypokalemiaDemonstration({ active, running: state.transport === 'running',
      patient: state.equipment?.resuscitation.renalHypokalemia, act: state.act,
    ...
    return <ScenarioProblemReport context={{
      ...
      canonicalUrl: 'https://opensimlab.com/renal-electrolyte/scenario/hypokalemia-magnesium-and-ongoing-losses',
    }} onOpen={state.pause} />;
  }
  ...
  it('records nine confirmations, freezes each reading and wait pause, and replays every full engine frame', () => {
    ...
    for (let batch = 0; batch < 1000 && !patient()?.ended; batch++) {
      ...
      const wait = current?.monitoringAtTick !== null && current?.potassiumDueInSeconds
        ? 'early' : current?.potassiumResponseObserved && current?.responseDueInSeconds ? 'later' : null;
    ...
    expect(recorded().map((action) => action.payload.action)).toEqual([
      'potassium', 'magnesium', 'call-support', 'review-context', 'manage-losses', 'monitor', 'reassess', 'reassess', 'handoff',
    ]);
    expect(recorded().every((action) => action.type === 'renal-hypokalemia-response')).toBe(true);
    const potassium = recorded().find((action) => action.payload.action === 'potassium')!;
    const magnesium = recorded().find((action) => action.payload.action === 'magnesium')!;
    const assessments = recorded().filter((action) => action.payload.action === 'reassess');
    expect(assessments[0]!.tick - potassium.tick).toBeGreaterThanOrEqual(EARLY);
    expect(assessments[1]!.tick - magnesium.tick).toBeGreaterThanOrEqual(RESPONSE);
    expect(patient()).toMatchObject({ ended: 'handoff', potassiumResponseObserved: true, responseObserved: true,
      recurrenceObserved: false, rapidPotassiumAttempted: false, monitoringStopAttempted: false,
  ...
  it('preserves accepted potassium and a paused clock after takeover while invalidating callbacks across reactivation', () => {
    ...
    expect(patient()!.potassiumAtTick).not.toBeNull();
    ...
    expect(recorded().map((action) => action.payload.action)).toEqual(['potassium', 'magnesium']);
    expect(patient()!.potassiumAtTick).toBe(before.patient!.potassiumAtTick); expect(finishes).toBe(0);
  });
  ...
  it('holds the real session during a shared report modal without reviving an already submitted confirmation', async () => {
    ...
    expect(advance).toBeUndefined(); expect(patient()!.potassiumDueInSeconds).toBeGreaterThan(0);
  ...
  it('re-observes recovery after recurrence even when earlier combined response credit is retained, and replays every frame', () => {
    begin(false);
    const choose = (action: RenalHypokalemiaAction) => act(() => session().act({ type: 'renal-hypokalemia-response', payload: { action } }));
    for (const action of ['potassium', 'magnesium', 'call-support', 'review-context', 'monitor', 'rapid-potassium', 'stop-monitoring'] as const) choose(action);
    const potassium = recorded().find((action) => action.payload.action === 'potassium')!;
    const magnesium = recorded().find((action) => action.payload.action === 'magnesium')!;
    for (let batch = 0; batch < 800 && session().tick < magnesium.tick + RESPONSE; batch++) frames();
    act(() => session().pause()); choose('reassess'); act(() => session().play()); frames();
    expect(patient()).toMatchObject({ responseObserved: true, observation: { potassiumMmolL: 3.1, magnesiumMmolL: 0.62 } });
    const historical = patient()!.observation;
    for (let batch = 0; batch < 800 && session().tick < potassium.tick + RECURRENCE; batch++) frames();
    act(() => session().pause()); choose('check-potassium'); choose('check-ecg'); act(() => session().play()); frames();
    expect(patient()).toMatchObject({ observation: historical, potassiumObservation: { potassiumMmolL: 2.5 },
      responseObserved: true, recurrenceObserved: false });
    act(() => session().pause()); choose('reassess'); act(() => session().play()); frames();
    expect(patient()).toMatchObject({ responseObserved: true, recurrenceObserved: true,
    ...
    const lossCare = recorded().find((action) => action.payload.action === 'manage-losses')!;
    const lastAssessment = recorded().filter((action) => action.payload.action === 'reassess').at(-1)!;
    expect(lastAssessment.tick - lossCare.tick).toBeGreaterThanOrEqual(RESPONSE);
    expect(patient()).toMatchObject({ ended: 'handoff', responseObserved: true, recurrenceObserved: true,
      rapidPotassiumAttempted: true, monitoringStopAttempted: true, durableRecoveryProven: false,
