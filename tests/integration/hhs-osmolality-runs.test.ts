/**
 * Reference transcripts for the HHS trajectory lesson, replayed through the real engine.
 *
 * The error path is an ordering error rather than a treatment error, because this
 * lesson delivers no treatment. Calling the pattern before the four-day trajectory
 * has been connected, then closing on the later report, is the mistake it is built
 * around, and the recovery path starts from exactly those refusals.
 */
...
import { hhsOsmolalityCompletionEvidence } from '../../src/modules/endocrine-metabolic/hhs-osmolality-completion';
import { hhsOsmolalityInlinePrompt } from '../../src/modules/endocrine-metabolic/tutor/hhs-osmolality-guidance';

type Choices = readonly (readonly [number, HhsOsmolalityAction])[];
const create = (region: 'US' | 'GB' = 'US') => new AnesthesiaEngine({ scenario: SCENARIO, seed: FIXTURES.seed, practiceRegion: region });
const choice = (tick: number, action: HhsOsmolalityAction): LearnerAction => ({ tick, type: 'hhs-osmolality-trajectory-response', payload: { action } });
...
function run(actions: Choices, until: number, level: GuidanceLevel = 'unassisted', region: 'US' | 'GB' = 'US') {
  ...
  for (let tick = 0; tick <= until; tick += 1) {
    ...
    const before = JSON.stringify(frame.equipment.resuscitation.endocrineHhsAssessment);
    const prompt = hhsOsmolalityInlinePrompt(level, { scenarioVersion: SCENARIO.metadata.version,
      hhsOsmolality: frame.equipment.resuscitation.endocrineHhsAssessment });
    if (level === 'unassisted') expect(prompt).toBeNull();
    expect(JSON.stringify(frame.equipment.resuscitation.endocrineHhsAssessment)).toBe(before);
  }
  expect(next).toBe(actions.length);
  return { events, hash: hash.digest('hex'), patient: engine.equipment().resuscitation.endocrineHhsAssessment! };
}

describe('HHS trajectory transcripts through the real engine and debrief', () => {
  it('binds exact content and observed state without upgrading pending clinical evidence', () => {
    ...
    expect(hhsOsmolalityCompletionEvidence({ ...SCENARIO, patient: { ...SCENARIO.patient, weightKg: 69 } }, ENGINE_VERSION, 'endocrine-metabolic')).toEqual([]);
  });
  ...
  it('refuses the recognize-first ordering and records nothing from it', () => {
  ...
  it('lets the same run recover, and keeps the refusals in the transcript', () => {
    ...
    expect(recovered.patient.hhsResolutionProven).toBe(false);
  });
});
