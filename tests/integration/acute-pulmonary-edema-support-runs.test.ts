/**
 * Reference transcripts for the pulmonary-edema support lesson, replayed
 ...
 * recognition that she is failing on support that is already running.
 */
...
import { apeSupportCompletionEvidence } from '../../src/modules/respiratory-medicine/acute-pulmonary-edema-respiratory-support-reassessment-completion';
...
type Choices = readonly (readonly [number, ApeSupportAction])[];
const create = (region: 'US' | 'GB' = 'US') => new AnesthesiaEngine({ scenario: SCENARIO, seed: FIXTURES.seed, practiceRegion: region });
const choice = (tick: number, action: ApeSupportAction): LearnerAction => ({ tick, type: 'acute-pulmonary-edema-respiratory-support-response', payload: { action } });
...
function run(actions: Choices, until: number, level: GuidanceLevel = 'unassisted', region: 'US' | 'GB' = 'US') {
  ...
  for (let tick = 0; tick <= until; tick += 1) {
    ...
    const before = JSON.stringify(frame.equipment.resuscitation.apeSupportAssessment);
    const prompt = apeSupportInlinePrompt(level, { scenarioVersion: SCENARIO.metadata.version,
      apeSupport: frame.equipment.resuscitation.apeSupportAssessment });
    if (level === 'unassisted') expect(prompt).toBeNull();
    expect(JSON.stringify(frame.equipment.resuscitation.apeSupportAssessment)).toBe(before);
  }
  expect(next).toBe(actions.length);
  return { events, hash: hash.digest('hex'), patient: engine.equipment().resuscitation.apeSupportAssessment! };
}

describe('Pulmonary-edema support transcripts through the real engine and debrief', () => {
  it('binds exact content and observed state without upgrading pending clinical evidence', () => {
    ...
    expect(apeSupportCompletionEvidence(SCENARIO, ENGINE_VERSION, 'respiratory-medicine')).toHaveLength(9);
    expect(apeSupportCompletionEvidence(SCENARIO, ENGINE_VERSION, 'obstetrics')).toEqual([]);
    expect(apeSupportCompletionEvidence(SCENARIO, 'changed', 'respiratory-medicine')).toEqual([]);
    expect(apeSupportCompletionEvidence({ ...SCENARIO, patient: { ...SCENARIO.patient, weightKg: 9 } }, ENGINE_VERSION, 'respiratory-medicine')).toEqual([]);
  });
  ...
  it('refuses the cause review before the failure has been named', () => {
    const errored = run(FIXTURES.commonError, FIXTURES.commonError.at(-1)![0] + 2);
    expect(errored.patient).toMatchObject({
      failureAtTick: null, wholePatientAtTick: null, escalationAtTick: null, handoffAtTick: null,
    });
    // Reviewing the precipitants is not the failure. Reviewing them before
    // naming what the mentation, effort and gas already say is.
  ...
  it('lets the same run recover, and certifies nothing at the end of it', () => {
    ...
    expect(recovered.patient.nivStartedByLearner).toBe(false);
    expect(recovered.patient.supportSettingSelected).toBe(false);
    ...
    expect(recovered.patient.pulmonaryEdemaAuthored).toBe(true);
