...
import { RENAL_HYPERKALEMIA_SHIFT_TICKS as EARLY,
  RENAL_HYPERKALEMIA_REMOVAL_TICKS as RESPONSE, RENAL_HYPERKALEMIA_REBOUND_TICKS as REBOUND,
...
describe('RenalHyperkalemia worked example through the real session pipeline', () => {
  ...
  const patient = () => session().equipment?.resuscitation.renalHyperkalemia;
  const worker = () => workers.at(-1)!;
  const recorded = () => sessionInternals().recorder!.build('pending').actions;
  function Harness({ active }: { active: boolean }) {
    const state = useSession();
    const demo = useRenalHyperkalemiaDemonstration({ active, running: state.transport === 'running',
      patient: state.equipment?.resuscitation.renalHyperkalemia, act: state.act,
    ...
    return <ScenarioProblemReport context={{
      ...
      canonicalUrl: 'https://opensimlab.com/renal-electrolyte/scenario/hyperkalemia-cardioprotection-and-rebound',
    }} onOpen={state.pause} />;
  }
  ...
  it('records ten confirmations, freezes each reading and wait pause, and replays every full engine frame', () => {
    ...
    for (let batch = 0; batch < 1000 && !patient()?.ended; batch++) {
      ...
      const wait = current?.monitoringAtTick !== null && current?.shiftDueInSeconds
        ? 'early' : current?.shiftResponseObserved && current?.removalDueInSeconds ? 'later' : null;
    ...
    expect(recorded().map((action) => action.payload.action)).toEqual([
      'calcium', 'shift', 'call-support', 'review-context', 'plan-removal', 'deliver-removal', 'monitor', 'reassess', 'reassess', 'handoff',
    ]);
    expect(recorded().every((action) => action.type === 'renal-hyperkalemia-response')).toBe(true);
    const shift = recorded().find((action) => action.payload.action === 'shift')!;
    ...
    expect(assessments[0]!.tick - shift.tick).toBeGreaterThanOrEqual(EARLY);
    expect(assessments[1]!.tick - removal.tick).toBeGreaterThanOrEqual(RESPONSE);
    expect(patient()).toMatchObject({ ended: 'handoff', shiftResponseObserved: true, removalResponseObserved: true,
      reboundObserved: false, ecgResolvedAttempted: false, glucoseMonitoringStopAttempted: false,
  ...
  it('preserves accepted calcium and a paused clock after takeover while invalidating callbacks across reactivation', () => {
    ...
    expect(patient()!.calciumAtTick).not.toBeNull();
    ...
    expect(recorded().map((action) => action.payload.action)).toEqual(['calcium', 'shift']);
    expect(patient()!.calciumAtTick).toBe(before.patient!.calciumAtTick); expect(finishes).toBe(0);
  });
  ...
  it('resumes after observed rebound with partial checks remaining historical, and replays every frame', () => {
    begin(false);
    const choose = (action: RenalHyperkalemiaAction) => act(() => session().act({ type: 'renal-hyperkalemia-response', payload: { action } }));
    for (const action of ['calcium', 'shift', 'reassess', 'ecg-resolved', 'stop-glucose-monitoring'] as const) choose(action);
    const shift = recorded().find((action) => action.payload.action === 'shift')!;
    frames(); const historical = patient()!.observation;
    for (let batch = 0; batch < 1700 && session().tick < shift.tick + REBOUND; batch++) frames();
    act(() => session().pause()); choose('check-ecg'); choose('check-glucose'); act(() => session().play()); frames();
    expect(patient()).toMatchObject({ observation: historical, ecgObservation: { rhythm: 'hyperkalemic-conduction' },
      glucoseObservation: { glucoseMgDl: 104 }, reboundObserved: false, shiftResponseObserved: false, removalResponseObserved: false });
    act(() => session().pause()); choose('reassess'); act(() => session().play()); frames();
    expect(patient()).toMatchObject({ reboundObserved: true, observation: { potassiumMmolL: 6.6 } });
    ...
    expect(lastAssessment.tick - removal.tick).toBeGreaterThanOrEqual(RESPONSE);
    expect(patient()).toMatchObject({ ended: 'handoff', removalResponseObserved: true, shiftResponseObserved: false,
