...
Additional longitudinal reassessment scenarios MAY declare an expected duration up to 360 simulated
...
- **THEN** the schema accepts that duration, rejects durations above 360 minutes, and the lesson offers pause and accelerated time

#### Scenario: A multi-hour treatment observation does not masquerade as a short simulated episode

- **GIVEN** a hypercalcemic-crisis lesson with an authored four-hour bridge-response checkpoint
- **WHEN** its duration appears in the catalog, briefing metadata, or structured data
- **THEN** it declares 240 simulated minutes rather than its shorter accelerated wall-clock time, states that the checkpoint is not a predicted response or a safe waiting interval, and supports pause and 60× observation
