...
- [x] 4.5 Assert published reference-individual parameters and at least one published concentration-time point per model — reference-individual parameters are asserted for every model. The concentration-time assertion is a qualitative profile check rather than a digitized point from a published figure, which is recorded in the validation report.
