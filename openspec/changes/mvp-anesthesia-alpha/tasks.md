...
- [ ] 4.5 Assert published reference-individual parameters and at least one published concentration-time point per model — Minto now uses the exact Part II Table 1 prediction; Schnider uses a Figure 5 prediction with a graph-resolution tolerance; Eleveld uses an arterial-plasma observation from official Supplement S1 with the source residual-error model. Marsh publishes no linked administration/covariate/sample record, so a separately computed matrix-exponential golden point was added but does not satisfy the literal published-point requirement. This remains open pending an approved requirement change or a qualifying Marsh source.
