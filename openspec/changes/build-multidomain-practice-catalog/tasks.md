...
- [ ] Wave B: complete 25 emergency-medicine and 24 critical-care scenarios.
  Emergency Medicine is available with 23 of 25 scenarios. These first 23 — undifferentiated,
  septic, hemorrhagic,
  ...
  with ECG change, severe hyponatremia with seizure, opioid toxicity, and exertional heat stroke
  reuse the shared deterministic session, monitor, transcript, action, and debrief frame; all 23 are
  completion-audited, prerendered, available offline, and explicitly bounded. The remaining 2 titles
  are scope only.
