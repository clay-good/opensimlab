...
- [ ] Wave B: complete 25 emergency-medicine and 24 critical-care scenarios.
  Emergency Medicine is available with 17 of 25 scenarios. These first 17 — undifferentiated,
  septic, hemorrhagic,
  ...
  tachycardia, unstable bradycardia, persistent VF arrest, PEA arrest, status epilepticus, and acute
  ischemic stroke reuse the shared deterministic session, monitor, transcript, action, and debrief
  frame; all 17 are completion-audited, prerendered, available offline, and explicitly bounded. The
  remaining 8 titles are scope only.
