...
- [ ] Wave B: complete 25 emergency-medicine and 24 critical-care scenarios.
  Emergency Medicine is available with 24 of 25 scenarios. These first 24 — undifferentiated,
  septic, hemorrhagic,
  ...
  with ECG change, severe hyponatremia with seizure, opioid toxicity, exertional heat stroke, and
  trauma primary survey reuse the shared deterministic session, monitor, transcript, action, and
  debrief frame; all 24 are completion-audited, prerendered, available offline, and explicitly
  bounded. The remaining 1 title is scope only.
