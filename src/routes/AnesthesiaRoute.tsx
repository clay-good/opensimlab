...
import { NEGATIVE_SCAN_ACTIONS, supportsNegativeScan } from '../modules/surgery-trauma/negative-scan';
import { RISING_REQUIREMENT_ACTIONS, supportsRisingRequirement } from '../modules/surgery-trauma/rising-requirement';
...
import { negativeScanReportActions } from '../modules/surgery-trauma/negative-scan-reporting';
import { risingRequirementReportActions } from '../modules/surgery-trauma/rising-requirement-reporting';
...
export interface ClinicalModuleConfig {
  readonly id: 'anesthesia' | 'emergency-medicine' | 'critical-care' | 'cardiology' | 'respiratory-medicine' | 'pediatrics' | 'neurology' | 'toxicology' | 'obstetrics' | 'neonatology' | 'endocrine-metabolic' | 'renal-electrolyte' | 'infectious-disease' | 'medical-surgical-nursing' | 'oncology' | 'surgery-trauma';
  readonly basePath: '/anesthesia' | '/emergency-medicine' | '/critical-care' | '/cardiology' | '/respiratory-medicine' | '/pediatrics' | '/neurology' | '/toxicology' | '/obstetrics' | '/neonatology' | '/endocrine-metabolic' | '/renal-electrolyte' | '/infectious-disease' | '/medical-surgical-nursing' | '/oncology' | '/surgery-trauma';
...
function collectReportRecentContext(session: SessionState, seed: number, sodiumLesson: boolean, avpLesson: boolean, refeedingLesson: boolean, diabetesLesson: boolean, renalLesson: boolean, hypokalemiaLesson: boolean, renalSodiumLesson: boolean, renalWaterLesson: boolean, renalCalciumLesson: boolean, renalMagnesiumLesson: boolean, meningococcalLesson: boolean, obstructionLesson: boolean, neutropeniaLesson: boolean, necrotizingLesson: boolean, endocarditisLesson: boolean, pneumoniaLesson: boolean, toxicShockLesson: boolean, possibleSepsisLesson: boolean,
  ...
  silentInteractionLesson: boolean, easyLabelLesson: boolean,
  negativeScanLesson: boolean, risingRequirementLesson: boolean): ScenarioReportRecentContext {
  const actions = sessionInternals().recorder?.build('pending').actions ?? [];
  return {
    seed: Math.trunc(seed),
    actions: risingRequirementLesson ? risingRequirementReportActions(actions, session.log)
      : negativeScanLesson ? negativeScanReportActions(actions, session.log)
      : easyLabelLesson ? easyLabelReportActions(actions, session.log)
      ...
      const lessonActions = action.type === 'rising-requirement-response' ? RISING_REQUIREMENT_ACTIONS
        : action.type === 'negative-scan-response' ? NEGATIVE_SCAN_ACTIONS
        : action.type === 'easy-label-response' ? EASY_LABEL_ACTIONS
    ...
    snapshot: {
      ...
      equipment: (sodiumLesson && !session.equipment?.resuscitation.hyponatremiaCorrection)
        ...
        || (negativeScanLesson && !session.equipment?.resuscitation.negativeScan)
        || (risingRequirementLesson && !session.equipment?.resuscitation.risingRequirement)
