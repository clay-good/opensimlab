...
export const MODULE_PROSE: Readonly<Record<string, ModuleProse>> = {
  ...
  'surgery-trauma': {
    description: 'Practice the surgical patient who never declares himself: a recovery that has quietly stopped, an investigation that cannot say no, an examination he could not take part in, a blood pressure that answers and will not hold, an injury whose severity is not visible on the evening it is assessed, a wait that nobody decided, a question two colleagues have already answered, a step attached to somebody\u2019s arrival, an explanation that excludes nothing, and a doubt you have not said out loud yet.',
