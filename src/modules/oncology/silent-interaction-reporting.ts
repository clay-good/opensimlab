...
import { SILENT_INTERACTION_ACTIONS, type SilentInteractionAction } from './silent-interaction';

const outcomes: Record<SilentInteractionAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'reconcile-what-she-is-actually-taking': { accepted: ['reconciled'], refused: [] },
  'record-the-interaction-and-its-direction': { accepted: ['direction-recorded'], refused: [] },
  ...
  'check-the-supplied-records': { accepted: ['records-check'], refused: [] },
  reassess: { accepted: ['initial-reassessment', 'reviewed-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'tell-her-to-stop-the-acid-tablets-today': { accepted: [], refused: ['stop-refused'] },
  'nothing-is-wrong-so-there-is-nothing-to-do': { accepted: [], refused: ['nothing-refused'] },
  'the-interaction-is-only-theoretical': { accepted: [], refused: ['theoretical-refused'] },
  'write-it-in-the-notes-and-move-on': { accepted: [], refused: ['notes-refused'] },
};

function choice(action: LearnerAction): SilentInteractionAction | undefined {
  if (action.type !== 'silent-interaction-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return SILENT_INTERACTION_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function silentInteractionReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `silent-interaction-${event}-${action.tick}`;
