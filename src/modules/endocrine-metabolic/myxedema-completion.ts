...
export function myxedemaCompletionEvidence(scenario: Scenario, capabilityVersion: string, moduleId: string): readonly CompletionRequirementAudit[] {
  if (moduleId !== 'endocrine-metabolic' || capabilityVersion !== '0.1.0-alpha.48'
    || scenario.metadata.id !== MYXEDEMA_FIXTURES.scenarioId || scenario.metadata.version !== '0.1.1'
    || MYXEDEMA_FIXTURES.contentVersion !== '0.1.1'
  ...
  return [
    ...
    { id: 'reference-transcripts', status: 'satisfied', evidence: ['myxedema-fixtures.ts binds expert, commonError, recovery, and noAction paths to content 0.1.1; model and real-engine tests verify distinct outcomes and deterministic replay.'] },
