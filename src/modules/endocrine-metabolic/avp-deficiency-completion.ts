...
export function avpDeficiencyCompletionEvidence(scenario: Scenario, capabilityVersion: string, moduleId: string): readonly CompletionRequirementAudit[] {
  if (moduleId !== 'endocrine-metabolic' || capabilityVersion !== '0.1.0-alpha.48'
    || scenario.metadata.id !== AVP_DEFICIENCY_FIXTURES.scenarioId || scenario.metadata.version !== '0.1.0'
    || AVP_DEFICIENCY_FIXTURES.contentVersion !== '0.1.0'
  ...
  return [
    ...
    { id: 'reference-transcripts', status: 'satisfied', evidence: ['avp-deficiency-fixtures.ts binds expert, commonError, recovery, and noAction to content 0.1.0 and seed 4919. Whole-state replay must remain identical without inferring that improving findings establish recovery.'] },
