...
export function hypoglycemiaCompletionEvidence(scenario: Scenario, capabilityVersion: string, moduleId: string): readonly CompletionRequirementAudit[] {
  if (moduleId !== 'endocrine-metabolic' || capabilityVersion !== '0.1.0-alpha.48'
    ...
    || scenario.metadata.version !== '0.1.1' || HYPOGLYCEMIA_FIXTURES.contentVersion !== '0.1.1'
  ...
  return [
    ...
    { id: 'reference-transcripts', status: 'satisfied', evidence: ['src/modules/endocrine-metabolic/severe-hypoglycemia-fixtures.ts binds expert, commonError, and recovery fixtures to content 0.1.1 and seed 4901. Real-engine replay tests verify distinct outcomes and guidance independence.'] },
    { id: 'guidance-and-demonstration', status: 'missing', evidence: [`Eight observed-state tutor rules at version ${HYPOGLYCEMIA_TUTOR_VERSION} provide Guided, Coached, and Unassisted behavior. An in-product expert demonstration is not yet bound to this lesson.`] },
