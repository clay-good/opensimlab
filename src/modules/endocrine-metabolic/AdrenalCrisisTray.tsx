...
import { adrenalInlinePrompt } from './tutor/adrenal-guidance';

export function AdrenalCrisisTray({ assessment, onAction, guidance = 'unassisted', onOpenSource }: {
  ...
  const prompt = adrenalInlinePrompt(guidance, { scenarioId: 'adrenal-crisis-treatment-before-tests', scenarioVersion: '0.1.0',
    adrenalCrisis: assessment, tick: 0, state: null, actions: [], ventilating: false, alarmCount: 0 });
  return <>
    {prompt && <aside className="syringe" aria-label="Private tutor">
      <div className="syringe__name">A moment to think</div>
      <p className="syringe__remaining">{prompt.suggestion}</p>
      <p className="syringe__remaining">{prompt.because} <a href={prompt.sourceHref} target="_blank" rel="noreferrer" onClick={onOpenSource}>Read the source (opens in a new tab)</a></p>
    </aside>}
    <section className="syringe" aria-labelledby="adrenal-rescue-title">
      ...
      {!assessment.ended && <div className="crisis-drug__actions">
        {assessment.hydrocortisoneAtTick === null && <Button onClick={() => onAction('hydrocortisone')}>Start qualified parenteral hydrocortisone</Button>}
        {assessment.salineAtTick === null && <Button onClick={() => onAction('saline')}>Start qualified isotonic saline pathway</Button>}
        ...
        {assessment.hydrocortisoneAtTick === null && <>
          <Button onClick={() => onAction('wait-for-cortisol')}>Wait for cortisol before steroids</Button>
          <Button onClick={() => onAction('oral-only')}>Rely on oral replacement only</Button>
        </>}
      </div>}
    </section>
    <section className="syringe" aria-labelledby="adrenal-reassess-title">
      ...
      {assessment.ended ? <p className="syringe__remaining">{assessment.ended === 'handoff' ? 'Practice complete: ongoing treatment and prevention ownership are handed off. This is not discharge clearance.' : 'Instructor takeover ended this branch. Open the debrief, then try a different rescue decision.'}</p> : <div className="crisis-drug__actions">
        <Button onClick={() => onAction('reassess')}>Reassess circulation and alertness</Button>
        {!assessment.recordReviewed && <Button onClick={() => onAction('review-record')}>Review replacement and laboratory record</Button>}
        {assessment.responseObserved && !assessment.preventionPlanned && <Button onClick={() => onAction('prevention')}>Plan steroid continuity and prevention</Button>}
        {assessment.preventionPlanned && <Button onClick={() => onAction('handoff')}>Hand off ongoing treatment and risk</Button>}
      </div>}
    </section>
  </>;
}
