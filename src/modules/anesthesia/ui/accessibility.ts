...
export function stateSummary(
  ...
  if (options.unspokenDoubt) {
    const patient = options.unspokenDoubt;
    // Three sides and one interval, because nothing measured here ever moves.
    lines.push(`Anaesthetised, draped, team brief completed. Consent form ${patient.consentSide}. Skin marking ${patient.markedSide}. Label on the displayed image ${patient.imageLabelSide}. Raised by anybody else: ${patient.raisedByAnyoneElse ? 'yes' : 'nobody'}. ${patient.incisionMade ? 'Incision made.' : 'No incision made.'}`);
    lines.push(`Everybody in the room is competent and paying attention to their own part of it. The consultant is scrubbed${patient.knifeRequested ? ' and has asked for the knife' : ' and has not asked for anything yet'}. Nobody has done anything wrong.`);
    lines.push('Supplied observations were pulse 78 per minute, blood pressure 110 over 64, ventilated at a set rate of 12 per minute, temperature 36.4 degrees Celsius, and oxygen saturation 99 percent. They will not change, and no number here was ever going to catch a label.');
    lines.push(`Current state: ${patient.alertness}.`);
    lines.push(`What you noticed stated: ${patient.noticedStatedAtTick === null ? 'no' : 'yes'}. Ways you might be wrong stated: ${patient.fallibilityStatedAtTick === null ? 'no' : 'yes'}. Cost of each mistake stated: ${patient.asymmetryStatedAtTick === null ? 'no' : 'yes'}. Said in the room: ${patient.spokenAtTick === null ? 'not yet' : 'yes'}. Bounded intent: ${patient.teamIntentAtTick === null ? 'not recorded' : 'recorded as the team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.knifeRequested) {
      lines.push('The consultant has asked for the knife. Nothing about the patient has changed and nobody has done anything wrong; the room is one step further along than it was.');
    }
    lines.push('A narrative synthesis of 31 studies from 4,822 screened found hierarchy, culture and education the most tested themes in challenging authority in the operating room. A scale study found self-efficacy, social outcome expectations and assertive attitude accounting for 73 percent of the variance in speaking up, with training level significantly associated with two of them. A systematic review of fourteen interventions found mixed results and specific doubt that education alone changes deeply rooted speaking-up behaviour; this rehearsal is one of those interventions. The re-check, the imaging, and whether the operation proceeds belong to the team; nothing is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: pulse ${patient.observation.heartRateBpm} per minute; consent ${patient.observation.consentSide}; marking ${patient.observation.markedSide}; image labelled ${patient.observation.imageLabelSide}; ${patient.observation.incisionMade ? 'incision made' : 'no incision made'}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('The consultant stopped, put the hand down and said thank you. Nobody was annoyed, and the team is re-checking the side against the consent, the mark and the imaging before anything else happens.');
    }
  }
  if (options.knownLabel) {
    const patient = options.knownLabel;
    // The label, the informant, and whether she is still here — the only things that matter.
    lines.push(`41-year-old man with a severe learning disability. Chronic constipation recorded for ${patient.labelYears} years. Informant of ${patient.informantYears} years ${patient.informantPresent ? 'is present' : 'has gone home'}. Abdominal examination he could take part in: ${patient.examinationAchieved ? 'achieved' : 'not achieved'}.`);
    lines.push('When he is distressed he is usually loud, agitated and self-injurious. Today he is quiet, rocking, keeping one hand flat on his abdomen, and will not sit down. He has eaten nothing since yesterday and refused the one thing he never refuses. His support worker says she has seen his constipation many times and this is not it.');
    lines.push('Supplied observations were pulse 104 per minute, blood pressure 138 over 82, respiratory rate 18 per minute, temperature 37.1 degrees Celsius, and oxygen saturation 97 percent on air. The pulse is a little fast and nothing here is alarming.');
    lines.push(`Current state: ${patient.alertness}.`);
    lines.push(`Label and what it explains recorded: ${patient.labelRecordedAtTick === null ? 'no' : 'yes'}. Account of what changed recorded: ${patient.carerAccountRecordedAtTick === null ? 'no' : 'yes'}. What the label cannot exclude recorded: ${patient.exclusionLimitsRecordedAtTick === null ? 'no' : 'yes'}. Surgical team: ${patient.escalationAtTick === null ? 'not asked' : 'asked'}. Bounded intent: ${patient.adjustmentIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.carerLeft) {
      lines.push('The support worker\u2019s shift has ended and a relief worker who has met him twice has arrived. Nothing about the patient has changed; what has left the department is the only comparison anybody had.');
    }
    lines.push('In 1,023 adults with intellectual disabilities the mean number of physical conditions was 11.04 and 98.7 percent were multimorbid, with constipation among the five most prevalent — so the label is very likely correct, and it is one of about eleven true labels, none of which excludes any other. A confidential inquiry into 247 deaths found 37 percent amenable to good healthcare against 13 percent in the general population, with carers not feeling listened to significant at p equals 0.006. The examination and its adjustments, any investigation, any prescribing, and whether he stays belong to the qualified team; nothing is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: pulse ${patient.observation.heartRateBpm} per minute; informant ${patient.observation.informantPresent ? 'present' : 'gone home'}; examination ${patient.observation.examinationAchieved ? 'achieved' : 'still not achieved'}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('The surgical team has answered, reads the constipation as explaining part of the picture rather than as excluding anything, and owns the examination with its adjustments, any investigation, and whether he stays.');
    }
  }
  if (options.deferredStep) {
    const patient = options.deferredStep;
    // Two clocks and one outstanding step, because nothing measured here ever moves.
    lines.push(`Open tibial fracture with bone visible through a six-centimetre contaminated wound. ${patient.minutesSinceInjury} minutes since injury and ${patient.minutesSinceArrival} since arrival. Antibiotic ${patient.antibioticGiven ? 'given' : 'not given'}. Anybody objecting: ${patient.objectionRecorded ? 'yes' : 'nobody'}. ${patient.pulsesPresent ? 'Foot warm, pulses palpable, sensation normal.' : 'Pulses not felt.'}`);
    lines.push('The board says orthopaedics will review her and give the antibiotics then, and the on-call registrar is in theatre with another case.');
    lines.push('Supplied observations were pulse 88 per minute, blood pressure 124 over 74, respiratory rate 16 per minute, temperature 36.7 degrees Celsius, and oxygen saturation 98 percent on air. Every one is normal and they will stay normal.');
    lines.push(`Current state: ${patient.alertness}.`);
    lines.push(`Injury and clock recorded: ${patient.injuryRecordedAtTick === null ? 'no' : 'yes'}. Step that is waiting recorded: ${patient.pendingStepRecordedAtTick === null ? 'no' : 'yes'}. What the interval is attached to recorded: ${patient.attachmentRecordedAtTick === null ? 'no' : 'yes'}. Team that can prescribe: ${patient.escalationAtTick === null ? 'not called' : 'called'}. Bounded intent: ${patient.prescribingIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.reviewSlipped) {
      lines.push('Theatre has rung: the registrar\u2019s case has become more complicated and the review will not happen before half past five. Nothing about the patient has changed, and the step attached to that review has moved with it.');
    }
    lines.push('In 137 type III open tibial fractures, antibiotics beyond 66 minutes from injury independently predicted deep infection at 90 days with an odds ratio of 3.78. In 230 open fractures elsewhere, administration beyond 120 minutes from arrival carried a 2.4-fold hazard, while the underlying median comparison of 61 against 83 minutes missed significance. Two thresholds measured from two different moments, both retrospective. Whether anything is given, which, how much and by what route, the dressing, and the debridement and its timing belong to the qualified team; nothing is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: pulse ${patient.observation.heartRateBpm} per minute; ${patient.observation.minutesSinceInjury} minutes since injury; antibiotic ${patient.observation.antibioticGiven ? 'given' : 'still not given'}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('The team that can prescribe has answered within minutes, did not need to see her first, states that the step does not have to wait for their review, and owns the prescription, the dressing and photography, and the debridement and its timing.');
    }
  }
  ...
  if (options.transientResponse) {
    const patient = options.transientResponse;
    // The shape and the clock, because here the single number really does move and the
    // direction that reassures is the one that misleads.
    lines.push(`Restrained driver, front of the car into a barrier at speed, ${patient.minutesSinceInjury} minutes ago. Abdomen tender and distending. Free fluid ${patient.freeFluidReported ? 'reported on a bedside scan already performed' : 'not reported'}. ${patient.bolusCount} warmed boluses given before this assessment.`);
    lines.push(`The first bolus took his pressure up and held it for ${patient.firstResponseHeldMinutes} minutes; the second took it up less far and held it for ${patient.secondResponseHeldMinutes}. Each answer was smaller and arrived sooner than the one before.`);
    lines.push(`Current state: ${patient.alertness}.`);
    lines.push(`Mechanism and clock recorded: ${patient.mechanismRecordedAtTick === null ? 'no' : 'yes'}. Shape of the response recorded: ${patient.responseRecordedAtTick === null ? 'no' : 'yes'}. What a picture can and cannot do recorded: ${patient.imagingLimitsRecordedAtTick === null ? 'no' : 'yes'}. Operating team: ${patient.escalationAtTick === null ? 'not called' : 'called'}. Bounded intent: ${patient.operativeIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.fallen) {
      lines.push('The pressure has fallen a third time, to 84 over 50 with a rate of 124, five minutes after an answer that held for nine minutes, and nothing was taken away to cause it.');
    }
    lines.push('In 243 registry patients hypotensive on arrival with isolated abdominal injury, the probability of death rose with every minute spent in the department, by as much as 0.35 percent a minute. Early whole-body imaging was associated with better survival across 4,621 blunt trauma patients, so the objection here is to the interval and the room rather than to imaging. The transfer, any imaging on the way or not at all, and the operation and its timing belong to the qualified team; no drug, dose, route, fluid, blood product, or procedure is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: pulse ${patient.observation.heartRateBpm} per minute; blood pressure ${patient.observation.systolicMmHg} over ${patient.observation.diastolicMmHg}; ${patient.observation.minutesSinceInjury} minutes since injury; answers holding ${patient.observation.firstResponseHeldMinutes} then ${patient.observation.secondResponseHeldMinutes} minutes.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('The operating team has answered, repeated the response pattern back, and owns the transfer, any imaging on the way or not at all, and the operation and its timing.');
    }
  }
  if (options.unfinishedSurvey) {
    const patient = options.unfinishedSurvey;
    // What the record says and what it rests on, in every state, because nothing else here moves.
    lines.push(`Motorcycle collision ${patient.hoursSinceInjury} hours ago, sedated and ventilated in intensive care. ${patient.listedInjuries} injuries listed from reported whole-body imaging. Secondary survey ${patient.secondarySurveyDocumented ? 'documented as complete' : 'not documented'}. Tertiary survey ${patient.tertiarySurveyDocumented ? 'documented' : 'not documented'}.`);
    lines.push('Supplied starting observations were pulse 88 per minute, blood pressure 118 over 68, ventilated at a set rate of 14 per minute, temperature 36.8 degrees Celsius, and oxygen saturation 98 percent. These remain historical starting observations, and none of them is abnormal.');
    lines.push('He was a Glasgow Coma Scale of 6 at the scene and could not report pain, move to command, or be asked where it hurt when the secondary survey was performed. The ward has asked whether he can be stepped down tonight.');
    lines.push(`Current state: ${patient.alertness}.`);
    lines.push(`Reasons the examination was limited recorded: ${patient.examinationLimitsRecordedAtTick === null ? 'no' : 'yes'}. What the injury list rests on recorded: ${patient.injuryListRecordedAtTick === null ? 'no' : 'yes'}. Absent tertiary survey recorded: ${patient.surveyIncompleteRecordedAtTick === null ? 'no' : 'yes'}. Trauma team: ${patient.escalationAtTick === null ? 'not asked' : 'asked'}. Bounded intent: ${patient.surveyIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.sedationLightened) {
      lines.push('Sedation has been lightened for the routine neurological assessment. He opens his eyes, localises to the right, does not move the left arm, and grimaces when the left forearm is handled. The ventilator, the drain, the pressures and every monitored number are exactly as they were, and the ward has asked a second time.');
    }
    lines.push('Surveying 399 injured patients prospectively turned a registry rate of 2 percent into 41 missed injuries in 36 patients, 9 percent, of which 21 were extremity fractures, and the commonest reason for missing them was an altered level of consciousness. Formalising the survey in 487 patients raised how often it was performed from 27 to 42 percent without reducing missed injury. Reexamination, re-review of the imaging, and any further imaging, referral, or operation belong to the qualified trauma team; no investigation, drug, dose, route, or procedure is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: pulse ${patient.observation.heartRateBpm} per minute; ${patient.observation.hoursSinceInjury} hours since injury; ${patient.observation.listedInjuries} injuries listed; tertiary survey ${patient.observation.tertiarySurveyDocumented ? 'documented' : 'still not documented'}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('The trauma team has answered, confirmed from its own record that no tertiary survey has been documented, and owns completing the survey, re-reviewing the imaging against it, and anything that follows.');
    }
  }
  ...
  if (options.silentInteraction) {
    ...
    // The absence is the presentation, so the summary has to say so rather than list normals.
    lines.push(`She feels well and there is ${patient.anyAbnormalFinding ? 'an abnormal finding' : 'no abnormal finding of any kind'}: every observation and every supplied result is normal. The medicines lists are the only place anything is wrong.`);
    lines.push('Supplied starting observations were pulse 74 per minute, blood pressure 124 over 78, respiratory rate 16 per minute, oxygen saturation 98 percent in air, and temperature 36.6 degrees Celsius. These remain historical starting observations.');
    lines.push(`Current state: ${patient.alertness}. The clinic list holds her targeted tablet alone; the general practice list holds four items including an acid tablet started ${patient.overlapWeeks} weeks ago; the community pharmacy list ${patient.pharmacyRecordArrived ? 'has arrived and holds six, including one she buys herself' : 'has been requested and has not arrived'}.`);
    lines.push(`Reconciliation done: ${patient.reconciledAtTick === null ? 'no' : 'yes'}. Direction of harm: ${patient.directionRecordedAtTick === null ? 'not recorded' : 'recorded'}. Treating team: ${patient.escalationAtTick === null ? 'not called' : 'called'}. Bounded intent: ${patient.treatmentIntentAtTick === null ? 'not recorded' : 'recorded as the treating team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    lines.push('Most of these targeted tablets need an acid stomach to dissolve, so acid suppression means less of the drug is absorbed and the harm runs towards less treatment rather than more toxicity. That is why nothing appears abnormal now and why nothing will appear abnormal later. Across 4,340 and 1,635 patients, concurrent acid suppression was associated with adjusted hazard ratios for death of 1.58, confidence interval 1.42 to 1.76, and 1.54, confidence interval 1.30 to 1.82, from retrospective database studies whose authors write association rather than causation. Whether the acid suppression continues, whether an alternative replaces it, and what happens to the targeted treatment belong to the treating team; no drug, dose, route, timing separation, or substitution is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: oxygen saturation ${patient.observation.spo2Percent} percent in air; ${patient.observation.symptomAccount}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('Her treating team has answered, owns the decision, and has asked for the dates of the overlap rather than the diagnosis.');
    }
    if (patient.choiceFeedback) lines.push(patient.choiceFeedback);
  }
  if (options.trialRule) {
    const patient = options.trialRule;
    // The cited rule and the condition it attaches, in the same breath, every time.
    lines.push(`Restaging at nine weeks reports new lesions and enlarging disease. The criterion being cited permits treating through that while the patient is clinically stable, and she is ${patient.clinicallyStable ? 'clinically stable' : 'not clinically stable'}.`);
    lines.push('Supplied starting observations were pulse 96 per minute, blood pressure 112 over 68, respiratory rate 22 per minute, oxygen saturation 94 percent in air, and temperature 36.9 degrees Celsius. These remain historical starting observations.');
    lines.push(`Current state: ${patient.alertness}. Over three weeks she has gone from managing her own shopping to needing help to wash, and has lost six kilograms.`);
    lines.push(`Trajectory recorded: ${patient.trajectoryRecordedAtTick === null ? 'no' : 'yes'}. What the criteria govern: ${patient.governanceRecordedAtTick === null ? 'not recorded' : 'recorded'}. Treating team: ${patient.escalationAtTick === null ? 'not called' : 'called'}. Bounded intent: ${patient.treatmentIntentAtTick === null ? 'not recorded' : 'recorded as the treating team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.documentRead) {
      lines.push('The cited criteria have been read. They govern trial data handling rather than patient management, they are not validated, and their allowance is conditional on clinical stability.');
    }
    lines.push(`Reported pseudoprogression rates do not exceed ${patient.pseudoprogressionCeilingPercent} percent and rest on small series. Hyperprogression is reported at between 4 and 29 percent, including 13.8 percent on immunotherapy against 5.1 percent on chemotherapy in one lung-cancer comparison, with nothing established to offer once it has happened. The published advice is to distinguish them so as to avoid both premature discontinuation and a delayed next line. Whether treatment continues, whether a further line is offered, and what she is told belong to the treating team; no drug, dose, route, cycle, interval, or line of therapy is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.`);
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: respiratory rate ${patient.observation.respiratoryRateBpm} per minute; oxygen saturation ${patient.observation.spo2Percent} percent in air; ${patient.observation.functionalAccount}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('Her treating team has answered, owns the decision, and will review her within days rather than at the eight-week scan.');
    }
    if (patient.choiceFeedback) lines.push(patient.choiceFeedback);
  }
  if (options.inheritedUrgency) {
    const patient = options.inheritedUrgency;
    // Findings first, always. The swelling is how this looks at every grade, including the safe ones.
    lines.push(`Facial and neck swelling with filled neck veins and chest-wall collaterals. The findings that would make this the grade that cannot wait are ${patient.emergencyFindingsPresent ? 'present' : 'not present'}: no stridor, fully alert, blood pressure unchanged.`);
    lines.push('Supplied starting observations were pulse 88 per minute, blood pressure 128 over 76, respiratory rate 18 per minute, oxygen saturation 96 percent in air, and temperature 36.8 degrees Celsius. These remain historical starting observations.');
    lines.push(`Current state: ${patient.alertness}. There is no tissue diagnosis, and the biopsy list is tomorrow morning.`);
    lines.push(`Grading findings recorded: ${patient.findingsRecordedAtTick === null ? 'no' : 'yes'}. The tissue decides the treatment: ${patient.tissueRecordedAtTick === null ? 'not recorded' : 'recorded'}. Diagnostic pathway: ${patient.pathwaySecuredAtTick === null ? 'not secured' : 'secured'}. Bounded intent: ${patient.treatmentIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.treatmentOffered) {
      lines.push('A treatment slot has been offered for tonight. Nothing about the patient has changed: what arrived is an offer, not a deterioration.');
    }
    lines.push(`Only about ${patient.lifeThreateningGradePercent} percent of patients with this syndrome present with the life-threatening grade, which is defined by significant cerebral oedema, significant laryngeal oedema, or significant haemodynamic compromise, and any of those is an indication for emergent intervention. Death is very rarely caused by the syndrome itself. A proportion is not this patient's risk and is not a reason to stop looking. Radiotherapy and its timing, stenting, systemic therapy, steroid and anticoagulation decisions belong to the qualified team; no drug, dose, route, product, or procedure is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.`);
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: respiratory rate ${patient.observation.respiratoryRateBpm} per minute; oxygen saturation ${patient.observation.spo2Percent} percent in air; ${patient.observation.stridor ? 'stridor present' : 'no stridor'}; ${patient.observation.consciousLevel}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('Acute oncology has accepted him, the biopsy is booked and flagged for the morning, and they say the histology determines which treatment he gets.');
    }
    if (patient.choiceFeedback) lines.push(patient.choiceFeedback);
  }
  if (options.loweringTheCount) {
    const patient = options.loweringTheCount;
    // Count and findings in the same sentence, always. The count alone is what is being refused.
    lines.push(`Supplied white cell count ${patient.whiteCellCount} times ten to the ninth per litre with blasts on the film, breathless at rest and confused. Leukostasis here is ${patient.leukostasisIsClinical ? 'a clinical designation, not a number' : 'defined by the count'}.`);
    lines.push('Supplied starting observations were pulse 108 per minute, blood pressure 108 over 62, respiratory rate 26 per minute, oxygen saturation 92 percent in air, and temperature 37.4 degrees Celsius. These remain historical starting observations.');
    lines.push(`Current state: ${patient.alertness}. There is no marrow result.`);
    lines.push(`Clinical picture recorded: ${patient.pictureRecordedAtTick === null ? 'no' : 'yes'}. What the count licenses: ${patient.licenceRecordedAtTick === null ? 'not recorded' : 'recorded'}. Haematology: ${patient.escalationAtTick === null ? 'not called' : 'called'}. Bounded intent: ${patient.treatmentIntentAtTick === null ? 'not recorded' : 'recorded as the qualified team\u2019s decision'}. Boundaries: ${patient.boundariesReviewedAtTick === null ? 'not reviewed' : 'reviewed'}.`);
    if (patient.clinicallyWorse) {
      lines.push('He is more breathless and harder to rouse than he was. The supplied count has not changed, because it is the same sample.');
    }
    lines.push('Across 13 retrospective studies of 1,743 patients, the risk ratio for early death with leukapheresis was 0.88, with a confidence interval from 0.69 to 1.13, and its authors argue against routine use. That interval includes benefit as well as harm. The cytoreduction strategy and its route, transfusion, prophylaxis and definitive treatment belong to the qualified team; no drug, dose, route, product, or procedure is selected here, and oxygen settings and exhaled carbon dioxide are not supplied in this lesson.');
    lines.push(patient.observation
      ? `Last requested full assessment at simulated ${formatElapsed(patient.observation.atTick)}: respiratory rate ${patient.observation.respiratoryRateBpm} per minute; oxygen saturation ${patient.observation.spo2Percent} percent in air; ${patient.observation.consciousLevel}.`
      : 'No new full assessment has been requested.');
    if (patient.teamObserved) {
      lines.push('Haematology has answered and is coming, and says that treating the leukaemia rather than any single manoeuvre on the count is what changes his outcome.');
    }
    if (patient.choiceFeedback) lines.push(patient.choiceFeedback);
  }
