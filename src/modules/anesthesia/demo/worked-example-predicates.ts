...
import { supportsExtubationReadinessDemonstration } from './extubation-readiness-demonstration';
...
import { supportsRenalHypomagnesemiaDemonstration } from '../../renal-electrolyte/demo/renal-hypomagnesemia-demonstration';
...
const WORKED_EXAMPLES: Readonly<Record<string, readonly ((scenario: Scenario) => boolean)[]>> = {
  ...
  anesthesia: [
    ...
    supportsExtubationReadinessDemonstration,
  ...
  'renal-electrolyte': [
    ...
    supportsRenalHypomagnesemiaDemonstration,
...
/**
 * The predicate table is the source of truth for which lessons have an example,
 * but nothing in the application imports this file.
 *
 * Importing it pulls all 242 demonstration modules, and each of those pulls its
 * lesson's narration. `Prebrief.tsx` and `AnesthesiaRoute.tsx` both need the
 * offer and both live in the shared cockpit chunk, so that one import put every
 * lesson's prose into every module's download — a lesson added to surgery and
 * trauma grew anesthesia. The offer only needs to know *whether* an example
 * exists, so `worked-examples.ts` answers from an inert list of keys instead and
 * `tests/unit/worked-example-offer.test.ts` holds the two in agreement.
 */
export function workedExampleKeys(
  catalogues: Readonly<Record<string, readonly Scenario[]>>,
): string[] {
  return Object.keys(WORKED_EXAMPLES).sort().flatMap((moduleId) => {
    const scenarios = catalogues[moduleId];
    if (!scenarios) throw new Error(`no scenario catalogue supplied for ${moduleId}`);
    return scenarios
      .filter((scenario) => WORKED_EXAMPLES[moduleId]!.some((supported) => supported(scenario)))
