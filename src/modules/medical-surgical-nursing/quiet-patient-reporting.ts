...
import { QUIET_PATIENT_ACTIONS, type QuietPatientAction } from './quiet-patient';

const outcomes: Record<QuietPatientAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'review-the-charted-impression': { accepted: ['impressions-reviewed'], refused: [] },
  'screen-for-arousal': { accepted: ['screened'], refused: [] },
  'record-the-screen-result': { accepted: ['result-recorded'], refused: ['result-refused'] },
  'escalate-on-the-positive-screen': { accepted: ['escalation-requested'], refused: ['escalation-refused'] },
  ...
  reassess: { accepted: ['initial-reassessment', 'screened-reassessment', 'reviewed-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'let-them-sleep-and-screen-later': { accepted: [], refused: ['deferral-refused'] },
  'quiet-is-settled': { accepted: [], refused: ['quiet-refused'] },
  'negative-earlier-screen-excludes': { accepted: [], refused: ['earlier-screen-refused'] },
  'call-it-low-mood': { accepted: [], refused: ['mood-refused'] },
};

function choice(action: LearnerAction): QuietPatientAction | undefined {
  if (action.type !== 'quiet-patient-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return QUIET_PATIENT_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function quietPatientReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `quiet-patient-${event}-${action.tick}`;
