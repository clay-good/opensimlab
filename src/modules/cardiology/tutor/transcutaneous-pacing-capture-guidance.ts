...
export const TRANSCUTANEOUS_PACING_CAPTURE_TUTOR_VERSION = '0.1.0';

export interface TranscutaneousPacingCapturePrompt {
 ...
 *
 * The reflex it works against is a screen that looks like it is working. The
 * pacer is capturing electrically at 70, every stimulus is followed by a broad
 * QRS and a distinct T wave, and she has no pulse — so the display shows a
 * treated bradycardia while the patient is in pulseless electrical activity.
 * The failure this lesson exists to prevent is not a wrong drug; it is the
 * minutes a team spends adjusting a pacer that is already doing its job.
 *
 ...
 */
export function transcutaneousPacingCaptureInlinePrompt(
  level: GuidanceLevel,
  input: { readonly scenarioVersion: string; readonly patient?: TranscutaneousPacingCaptureProgress },
): TranscutaneousPacingCapturePrompt | null {
