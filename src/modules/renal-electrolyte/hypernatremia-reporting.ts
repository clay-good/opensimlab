...
import { RENAL_HYPERNATREMIA_ACTIONS, type RenalHypernatremiaAction } from './hypernatremia';

const outcomes: Record<RenalHypernatremiaAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'restore-volume': { accepted: ['volume-restoration'], refused: [] },
  'replace-water': { accepted: ['water-replacement'], refused: ['water-review-refused'] },
  'manage-losses': { accepted: ['losses-care'], refused: ['losses-review-refused'] },
  'assist-water-access': { accepted: ['water-access'], refused: [] },
  ...
  'check-fluid-balance': { accepted: ['fluid-balance-check'], refused: [] },
  reassess: { accepted: ['initial-reassessment', 'volume-reassessment', 'water-reassessment', 'combined-reassessment', 'recurrence-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'empiric-desmopressin': { accepted: [], refused: ['desmopressin-refused'] },
  'normalize-now': { accepted: [], refused: ['normalization-refused'] },
};

function choice(action: LearnerAction): RenalHypernatremiaAction | undefined {
  if (action.type !== 'renal-hypernatremia-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return RENAL_HYPERNATREMIA_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function renalHypernatremiaReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `renal-hypernatremia-${event}-${action.tick}`;
