...
import { RENAL_RHABDOMYOLYSIS_ACTIONS, type RenalRhabdomyolysisAction } from './rhabdomyolysis';

const outcomes: Record<RenalRhabdomyolysisAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'review-cause': { accepted: ['cause-review'], refused: [] },
  'examine-compartments': { accepted: ['compartment-examination'], refused: [] },
  'review-number': { accepted: ['number-review'], refused: [] },
  'review-additions': { accepted: ['additions-review'], refused: [] },
  'arrange-fluids': { accepted: ['fluid-ownership'], refused: [] },
  'call-support': { accepted: ['support'], refused: [] },
  monitor: { accepted: ['monitoring'], refused: [] },
  'check-creatine-kinase': { accepted: ['creatine-kinase-check'], refused: [] },
  'check-renal': { accepted: ['renal-check'], refused: [] },
  reassess: { accepted: ['initial-reassessment', 'unexamined-reassessment', 'serial-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'dialyse-on-number': { accepted: [], refused: ['dialysis-refused'] },
  'add-bicarbonate-and-mannitol': { accepted: [], refused: ['additions-refused'] },
};

function choice(action: LearnerAction): RenalRhabdomyolysisAction | undefined {
  if (action.type !== 'renal-rhabdomyolysis-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return RENAL_RHABDOMYOLYSIS_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function renalRhabdomyolysisReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `renal-rhabdomyolysis-${event}-${action.tick}`;
