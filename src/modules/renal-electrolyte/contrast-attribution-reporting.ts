...
import { RENAL_CONTRAST_ACTIONS, type RenalContrastAction } from './contrast-attribution';

const outcomes: Record<RenalContrastAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'review-label': { accepted: ['label-review'], refused: [] },
  'review-alternatives': { accepted: ['alternatives-review'], refused: [] },
  'review-evidence': { accepted: ['evidence-review'], refused: [] },
  'withdraw-exposures': { accepted: ['exposures-withdrawn'], refused: [] },
  ...
  reassess: { accepted: ['initial-reassessment', 'unexamined-reassessment', 'record-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'attribute-to-contrast': { accepted: [], refused: ['attribution-refused'] },
  'stop-looking': { accepted: [], refused: ['stop-looking-refused'] },
};

function choice(action: LearnerAction): RenalContrastAction | undefined {
  if (action.type !== 'renal-contrast-attribution-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return RENAL_CONTRAST_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function renalContrastAttributionReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `renal-contrast-${event}-${action.tick}`;
