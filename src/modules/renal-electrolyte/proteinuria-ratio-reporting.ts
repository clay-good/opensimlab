...
import { RENAL_PROTEINURIA_ACTIONS, type RenalProteinuriaAction } from './proteinuria-ratio';

const outcomes: Record<RenalProteinuriaAction, { accepted: readonly string[]; refused: readonly string[] }> = {
  'compare-variation': { accepted: ['variation-comparison'], refused: [] },
  'review-sampling': { accepted: ['sampling-review'], refused: [] },
  'review-patient': { accepted: ['patient-review'], refused: [] },
  'request-repeat': { accepted: ['repeat-requested'], refused: [] },
  'own-decision': { accepted: ['decision-owned'], refused: [] },
  'call-support': { accepted: ['support'], refused: [] },
  monitor: { accepted: ['monitoring'], refused: [] },
  'check-ratio': { accepted: ['ratio-check'], refused: [] },
  'check-clinical': { accepted: ['clinical-check'], refused: [] },
  reassess: { accepted: ['initial-reassessment', 'uncompared-reassessment', 'repeat-reassessment'], refused: [] },
  handoff: { accepted: ['handoff'], refused: ['handoff-refused'] },
  'change-treatment': { accepted: [], refused: ['change-treatment-refused'] },
  'call-it-progression': { accepted: [], refused: ['progression-refused'] },
};

function choice(action: LearnerAction): RenalProteinuriaAction | undefined {
  if (action.type !== 'renal-proteinuria-ratio-response' || !Number.isSafeInteger(action.tick) || action.tick < 0
  ...
  return RENAL_PROTEINURIA_ACTIONS.find((value) => value === descriptor.value);
}

/** Only uniquely attributable outcomes enter optional context; never event prose or arbitrary payloads. */
export function renalProteinuriaRatioReportActions(actions: readonly LearnerAction[], events: readonly EngineEvent[]): ScenarioReportActionContext[] {
  ...
  return actions.slice(-REPORT_CONTEXT_ACTION_LIMIT).flatMap((action) => {
    ...
    const id = (event: string) => `renal-proteinuria-${event}-${action.tick}`;
